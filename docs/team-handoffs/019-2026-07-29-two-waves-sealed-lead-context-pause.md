...
2. **⛔ THE COMMIT DISCIPLINE IN HANDOFF 018 IS DEFECTIVE — DO NOT INHERIT IT.** The three-step form (`diff --cached` before · chained `add && commit` · `show --stat` after) has **no gate**: `&&` chains add→commit; **nothing chains check→add**. A pre-check printed a dirty index correctly and completely and the commit ran anyway, sweeping 14 of another implementer's staged files into a docs commit. ⇒ **A check whose output the actor reads only AFTER the action is a RECEIPT, not a gate.**
   ⭐ **CORRECTED FORM: `git commit -F <msg> -- <explicit paths>`.** A foreign staged **FILE** structurally cannot enter.
   ⛔⛔ **AND HERE IS ITS LIMIT — DO NOT ADOPT THE FORM WITHOUT IT. `git commit -- <path>` FILTERS BY PATH, so it does NOTHING about a concurrent writer editing the SAME path.** Unrepresentability holds for foreign **files**; it does **not** hold for foreign **changes to your own files** — so for `IMPLEMENTATION_PLAN.md`, or an area's `LESSONS.md`/`CLAUDE.md`, **the pathspec buys nothing at all.** The only mitigations there are the old ones: **commit promptly** so the window is seconds wide, and **read `git diff <path>` before committing it.**
   ⚠ **The failure mode is SUBSTITUTION, not ignorance:** *"I pathspec-limited it"* stands in for the check it does not perform. **A PARTIAL GUARANTEE IS MORE DANGEROUS THAN NONE WHEN IT RETIRES THE HABIT THE REST OF THE RISK NEEDED.** Proven inside one day: the orchestrator swept the lead's uncommitted tracker bullet *while using the pathspec form*, and separately committed over a RED `plan-lint` one commit after banking the fix for exactly that. **This is L103's limits-clause proving itself against L103's own remedy.**
   ⚠ Minor clause, also required: a NEW file still needs `git add` first — the add makes it *visible*, the pathspec on the *commit* remains the *filter*. Without it someone concludes the form is broken and reverts to the detector.
