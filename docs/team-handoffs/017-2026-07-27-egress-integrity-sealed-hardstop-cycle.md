...
Activated because: handoff 017. FIRST LEG — re-point the synthesis corpus to 13.8j's namespaced stub paths. ⚠ The corpus LOADS FINE — there is NO hash_mismatch (an earlier claim of one was false, produced by hashing raw bytes instead of using the loader's canonicalizing computation, and without ever running the suite). `pnpm vitest run` in packages/evals: 607 pass / 1 fail / 14 skipped; the skips are the API-key-gated real-Claude eval and are unrelated. ONE issue: `entity-create-stub` + `stub-plus-newregion-auto` fail `create_paths` + `stubs_present`.
