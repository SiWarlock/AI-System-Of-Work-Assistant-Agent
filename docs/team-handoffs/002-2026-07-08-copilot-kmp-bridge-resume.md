...
2. ✅ **DONE (session 050, `8f45943`)** — G3 providers SDK-MCP adapter `createCopilotProposeKnowledgeMcpServer` (`packages/providers/src/runtime/copilot-propose-knowledge-mcp.ts`): exposes `mcp__copilot__propose_knowledge` over an injected structural handler (args-as-`unknown`); reuses the shared "copilot" server name ⇒ tool id reconciles to catalog `copilot.propose_knowledge`. Review SHIP (tool-id correspondence traced sound); folded a MEDIUM (SDK `z.object` strips unknown keys before the handler — containment rests on server-derived path/workspace). ⚠ shipped mirror carries the same header imprecision (small follow-up).
3. **G4 — runner + boot flag** `copilotProposeKnowledge` (OFF; mirror `copilotProposeMode`). ← **NEXT.** Runner MUST: (a) server-bind `workspaceId`; (b) resolve `noteExists` at call time via `projectNotePath(workspaceId, projectId)`; (c) inject BOTH `createNoteProjectIdReader` AND `createNoteExistsProbe` (built from the SAME WS-8 `WorkspaceNoteRead`) into the executor deps (`noteExists` is required); (d) close `handleCopilotProposeKnowledgeToolCall` over its server-bound {workspaceId, sourceRef, noteExists, sink} and pass it as the G3 adapter's handler; (e) route the semantic dispatch into `dispatchApproval` via `createApprovalDispatchRouter`. ⚠ G3-review LOW: both propose servers are named "copilot" — if a job ever wants BOTH propose tools, compose them as two `tools` in ONE `createSdkMcpServer({name:"copilot", tools:[…]})`, never two servers under one map key.
