...
- **Governance language is calm and specific:** "Approve", "Defer 24h", "needs close-out", "no duplicate", "transcript pending", "reattach source".
- **⛔ Safety-posture copy is DERIVED copy, never decoration (added 2026-07-26).** Words that assert a governance GUARANTEE — "local-only", "zero-egress", "encrypted", "nothing leaves this Mac" — may appear ONLY where the rendered value comes from the state that governs it, and only at the scope that state applies to. `local-only` was previously listed here as approved copy and was implemented as an unconditional top-bar pill; it asserted local-only on every screen while employer and personal content egressed to Claude. Egress posture is PER-WORKSPACE and lives in the workspace-settings egress pane. If a screen cannot name the record its claim is derived from, the claim does not go on that screen.
