...
- **Sidebar structure — Option B (config tucked in Settings):**
  ...
  - **Governance-legibility guardrail:** ⛔ AMENDED 2026-07-26 — an always-on governance SIGNAL must still be DERIVED and stated at the scope it applies to; the egress pill violated both (a global constant for a per-workspace fact) and was removed. First-class visibility means reachable-and-truthful, never a chrome badge asserting what chrome cannot know. config *pages* live under Settings, but the always-on governance *signals* stay first-class and visible — **Health** in the main rail (with alert dot), System Health on Today. Keeps the rail calm without hiding the guarantees. ⛔ **The egress pill is NOT among them and must not be re-added here** (removed 2026-07-26; see lines 58 and 84 of this file, and `ui-ux-spec.md`) — this enumeration formerly still listed it, contradicting its own amendment two sentences above. *(corrected 2026-08-11, owner-authorized, from 24.6 round 2 finding DOC-1/F7 → task 24.10.)*
