...
>
> ⭐ **`L109`: a check you read AFTER the action is a receipt, not a gate** — so run the "after" pass **before** `git commit`, and treat a non-zero exit as a stop, not a note. **A pre-commit hook is the stronger form and is tracked as a task; until it exists, this line is the gate.**
