...
**Version:** v0.2 Draft  **Date:** 2026-06-27  **Supersedes:** v0.1 (faithful transcription of the PDF; see `system_of_work_assistant_prd_v0_1.md`)
...
- **Runtimes.** Claude Agent SDK named the default reference runtime; Hermes a required, DoD-tested but install-opt-in adapter (new §13.5); Temporal the sole scheduler (RT-7).
...
- 22. References
- Appendix A. Proposed Edits Pending Owner Sign-off
...
| Retrieval usefulness | GBrain search/think returns relevant project/person/decision context for 90%+ of benchmark queries (verified by KN-10). |
...
| KN-10 | GBrain retrieval shall return at least one relevant project/person/decision/source context item for 90%+ of a fixed benchmark query set (the §5.4 benchmark query set; corpus assembled in Phase 0). | P0 |
...
| RT-6 | Hermes cron and Kanban may be supported through the Hermes adapter but shall not replace Temporal as the system workflow source of truth. (See RT-7: on local-first V1, Hermes cron/Kanban autonomy is off by default and Hermes runs only within Temporal-owned jobs.) | P1 |
| RT-7 | Temporal shall be the only scheduler permitted to trigger cross-system workflows or external side effects. On the single local Mac, Hermes cron and Hermes Kanban autonomy are DISABLED by default in V1; Hermes — including Kanban and subagent fan-out (§13.4) — is invoked only as a runtime capability inside a Temporal-owned job, never as an independent scheduler (amends RT-6). GBrain Minions cron is restricted to brain-internal maintenance (sync, embed, extract, dream, health) and shall not issue external writes or KnowledgeWriter mutations except through the controlled bridge (RT-5, §14.4, KN-9). The control plane shall enforce a single active instance and cap concurrent agent runs against a single-machine resource budget (reusing §13.1 maxRuntimeSeconds/maxCostUsd job caps and §21 cost controls). | P0 |
...
| ID | Requirement | Priority |
|---|---|---|
| LIFE-1 | Single active instance via an exclusive lease (file lock or Temporal task-queue lease); a second launch attaches read-only or refuses to start workers, never a second scheduler; a stale lock is recoverable on restart. | P0 |
...
- Treat Hermes cron and Kanban as runtime capabilities that are OFF by default in V1 and invoked only within Temporal-owned jobs (RT-7); they are not autonomous schedulers and not the product workflow source of truth. Kanban/subagent fan-out (§13.4) remains available when driven by a Temporal-owned job.
...
- What is the default retention policy for raw transcripts and audio-derived materials? (see Appendix A, item 2)
...
These eight items were surfaced and verified by the v0.2 review but are **NOT applied** to the body above because each would add new V1 build scope or touch a settled product/privacy decision. The "keep full DoD" directive forbids cutting scope; it does not authorize unbounded new P0 work without a decision. Each is presented for accept / defer-to-V1.1 / reject.
...
- **RT-7 disables Hermes cron/Kanban autonomy by default (Conflict 7).** RT-7 (applied) makes Temporal the sole scheduler and turns OFF Hermes cron/Kanban autonomy on the local Mac — Hermes is still invoked *within* Temporal-owned jobs, and no capability is removed, but this flips a default on a runtime you reportedly run continuously. If you want Hermes cron usable standalone, its side effects would need to route through the Tool Gateway/idempotency layer (a larger change; not recommended for V1). **Flagging for awareness; applied as the safe default.**
