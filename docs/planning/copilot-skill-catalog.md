...
| Direct page / chunk / slug read (`get_page`, `get_chunks`, `resolve_slugs`, `obsidian_read_note`) | ✓ | ✓ | built | 13.4 / 13.10d | **`vault.read` BUILT-DORMANT ✅ (§13.10d, boot flag `copilotVaultRead` OFF)** — the `handleCopilotVaultReadCall` handler (SYMLINK-SAFE: lexical WS-8 + traversal → realpath containment + REAL-path re-attribution → read; fail-closed, never-throws) + `createCopilotVaultMcpServer` (`mcp__vault__read`, coexists with the gbrain proxy) + runner/boot wiring. Dual-reviewer: a symlink-escape CRITICAL was found + fixed in-slice (realpath re-attribution) + regression-tested. | Owner-flip `copilotVaultRead` (needs the Obsidian vault at `vaultRoot`). `get_chunks`/`resolve_slugs` are gbrain reads (separate). |
| Graph neighborhood read (`get_backlinks`/`get_links`/`get_tags`/`get_timeline`/`traverse_graph`) | ✓ | ✓ | built | 13.4 / 13.10d | **gbrain side DONE ✅** (gate d `245e4fd`: cataloged `gbrain.traverse_graph`/`gbrain.get_timeline` at their real live MCP names; DORMANT behind `copilotAgentMode`, WS-8 gate a still open) | vault backlinks tool still from 13.4. Both take a model-suppliable `slug` → covered by the WS-8 go-live gate. |
| Brain / vault introspection & health — READ half (`get_stats`/`get_health`/`get_versions`/`whoami`, `vault_health`) | ✓ | ✓ | excluded | 13.4 / 13.10d | **gbrain side EXCLUDED** (gate d: `get_health`/`get_stats` require ADMIN scope — unreachable to the read-pinned DCR client; the old phantom `gbrain.health`/`schema_read` ids were pruned. `vault_health` still IN-PLAN via 13.4) | Diagnostics only; the read Copilot has no need for brain health/stats (a separate worker path surfaces degraded health). Catalog the vault `vault_health`/`validate_note` read from 13.4. |
| **Graph anomaly / contradiction / orphan detection** (`find_contradictions`/`find_anomalies`/`find_orphans`) | ✓ | ✓ | built | 13.10b | **BUILT-DORMANT ✅** (`b186d82`) | Highest-leverage *no-inference* safety add (REQ-F-017): surface conflicts BEFORE answering, route to clarification instead of guessing. Take gbrain's READ shape, never osb `/reconcile` auto-resolve. (`traverse_graph` is now cataloged at its real name — gate d `245e4fd`, superseding the old phantom `gbrain.graph`, see §5.2(d).) |
...
| Skill self-introspection (`list_skills` / `get_skill`) | ✗ | ✓ | built | 13.10d | **BUILT-DORMANT ✅** (`bf994e6` policy + `bdf4170` worker + `d693e32` providers/wiring, boot flag `copilotSkillIntrospection` OFF) | *(Critique addition.)* The agent enumerating which read-skills it can invoke + reading one skill's metadata — the exact C6 skill-catalog-over-MCP pattern, zero write risk. Cataloged `skills.list`/`skills.get` + a NEW 4th `workspace-agnostic` scoping class (they read the STATIC catalog, touch no workspace data ⇒ nothing to scope, no leak, kept on any brain). `handleCopilotSkillIntrospect` NEVER reveals the write-proposing tool (reads `COPILOT_READ_TOOLS`, not the combined `CATALOG`) + never-throws. `createCopilotSkillsMcpServer` (`mcp__skills__*`) mirrors the vault/gbrain-proxy MCP. Dual-reviewer clean (security 0 crit/high/med). |
...
| Schema READ (`schema_read` / `gbrain schema show`) | ✓ | ✗ | not-exposed | — | NOT exposed (the phantom `gbrain.schema_read` was pruned in gate d — no live MCP tool) | The read-only slice of the brain-administration row (§3.6). Diagnostics only; the authoring/migration siblings are `not-a-copilot-skill`. |
...
| Capability | gbrain | osb | Code | Plan | BUILD STATUS | Machinery / recommendation |
|---|---|---|---|---|---|---|
| **Multi-hop cited synthesis / Copilot Q&A** (`think`, `contained_synthesis`; osb thinking-tools) | ✓ | ✓ | built | partial | **BUILT** (§9.6 Copilot live end-to-end via the completion/synthesis path — NOT a gbrain `contained_synthesis` tool; that phantom id was pruned in gate d `245e4fd` as it has no live MCP tool) | This IS the shipped §9.6 Copilot. Enhance with `think`'s explicit "what the brain does NOT yet know" + conflict output → directly serves REQ-F-017 no-inference. 13.3 feeds it. |
...
- **Tier 1 — CATALOG-EXTEND READ-ONLY** (biggest capability-per-risk win). The missing gbrain analysis reads + 13.4's 5 vault read tools + `list_skills`/`get_skill`. — **BUILD: gbrain-read portion DONE ✅** (`b186d82` / `9c0ba53` / `24174a3` — 14 analysis/intelligence reads + `get_recent_salience`, all BUILT-DORMANT). **`vault.read` + `list_skills`/`get_skill` DONE ✅** (§13.10d — the vault page-read handler + the skills-introspection MCP, both behind default-OFF flags). **Remaining:** the rest of 13.4's vault-MCP read surface (`obsidian_search`/`read_note`/`backlinks`/`vault_health`/`validate_note`) — need the Obsidian vault path wired.
...
- **(b) `serve --http` op scoping. ✅ VERIFIED (session 042).** Probed live vs gbrain v0.35.1: `serve --http` advertises all 63 tools to any client but enforces per-op scope (read/write/admin) at INVOCATION; scope is client-asserted at DCR REGISTRATION (`createGbrainDcrTokenProvider` pins `scope=read`). The frozen `GbrainReadGrant.allowedOps` enum is a **SoW-side Path-1 gate gbrain never sees**, and the agentic Path-2 allow-list bypasses it → **growing the frozen enum is NOT required** for the analysis reads; its truthing is an OPTIONAL Appendix-A cleanup, not a blocker.
- **(c) C6 governance eval** (`packages/evals` = eval-security track — coordinate). **✅ NOTED (session 042):** the 5-point read-path eval contract is in `docs/runbooks/copilot-propose-go-live.md` §13.10 gate (c) — WS-8 no-cross-workspace-leakage per cataloged read (query-scope AND result-content), the UI-safe gate strips `find_contradictions.resolution_command`, dormant-until-scoped, fail-safe on unknown tools. WS-8 assertions firm up once gate (a)'s mechanism lands.
- **(d) Phantom-name cleanup. ✅ DONE (session 042, `245e4fd`).** Renamed `gbrain.graph`→`gbrain.traverse_graph` and `gbrain.timeline`→`gbrain.get_timeline` (real live read tools); pruned `gbrain.schema_read`/`gbrain.contained_synthesis` (no live MCP tool) + `gbrain.health` (real op `get_health` requires ADMIN scope, unreachable to the read-pinned client). Servable-under-read-scope is now a catalog precondition; fail-safe preserved.
