...
- **SC5–SC9 NEXT** — the rest of the dormant P2 agentic layer (all behind `copilotAgentMode` OFF): **SC5** the pure arg/result cores (`policeGbrainToolArgs` deny `__all__`/`all_sources` + pin source_id/slugPrefix/seed-slug; `redactGbrainToolResult` drop foreign hits — **MUST fold A2** traverse_graph edge+target filtering, **A3** find_contradictions fail-closed far-side, **A4** strip `resolution_command` + page-naming fields server-side) · **SC6** transport seams (optional argPolicer + PostToolUse hook on the C2 transport) · **SC7** the in-process gbrain-proxy MCP server (PRIMARY delivery) · **SC8** runner wiring (allow-list from `copilotScopedReadToolIds`, proxy/policer bound to the served workspace) · **SC9** the admission backstop (`admitCopilotAgentJob` rejects an unscopable tool for the partition state). SC5 is the biggest.
