...
- **SC5b NEXT (the biggest, most security-critical slice)** — the result redactor `redactGbrainToolResult(mcpToolName, result, scope)`: per-tool result-shape parsing (the gbrain MCP result is `{content:[{type:"text", text:"<JSON>"}]}`), drop foreign hits by per-hit slug, and fold the RESULT-LEAKAGE fixes — **A2** traverse_graph filter nodes AND link edges/targets (+ strip link `context`), **A3** find_contradictions drop a pair if EITHER side foreign with a FAIL-CLOSED far-side (unattributable ⇒ foreign ⇒ drop), **A4** strip `resolution_command` + every page/path/title-naming field server-side (keep only severity/axis/confidence + opaque in-workspace refs). Malformed result ⇒ drop-all.
- **SC6–SC9** — **SC6** transport seams (optional argPolicer + PostToolUse hook on the C2 transport) · **SC7** the in-process gbrain-proxy MCP server (PRIMARY delivery) · **SC8** runner wiring (allow-list from `copilotScopedReadToolIds`, proxy/policer/redactor bound to the served workspace) · **SC9** the admission backstop.
