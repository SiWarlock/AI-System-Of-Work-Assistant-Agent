...
| DB schema source | Single-dialect Drizzle `sqlite-core` source + portable column types; pure repo interfaces; pg-core mirror + both-dialect contract suite deferred to Phase 2 | Locked (2026-06-30, ADR-009 — **completed in Phase 2**: pg-core mirror + both-dialect contract suite landed) | SQLite is the V1 default (§13); Drizzle has no truly dialect-neutral builder, so author the portable source now and mirror at adapter time | Dual per-dialect table defs from a shared column-spec factory |
| Postgres test target | PGLite (in-process real PG16) is the "real Postgres" for the both-dialect contract suite; optional `postgres:16` Docker gate (`SOW_PG_DOCKER=1`, skipped by default); prod driver = node-postgres (`pg`) | Locked (2026-06-30) | Satisfies §12 "real, not mocked" server-free/CI-friendly (PGLite = the same PG engine GBrain uses); Docker gate for full-server fidelity before DoD | Docker-only, or defer the Postgres run |
| DB storage ports | Async `Promise<Result>` for MigrationEngine / backup / restore ports + orchestrators | Locked (2026-06-30) | PGLite's driver is async end-to-end; the only callers were the lifecycle/backup tests | Keep sync + a sync PG shim |
