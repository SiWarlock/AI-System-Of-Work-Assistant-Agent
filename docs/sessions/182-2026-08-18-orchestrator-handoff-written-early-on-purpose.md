...
- ✅ **THE `L103`-CONSISTENCY QUESTION — ANSWERED AND CLOSED** (knowledge, `0cf00474` → doc `179` §5a; mirrored to `### 24.103` at `84a295f0`). ⚠ **This line said *"may still be unanswered"* when written 40 minutes ago — corrected here rather than left, because a handoff carrying a false open item is the exact defect this document opens by naming.** ⭐⭐ **THE ANSWER INVERTS THE QUESTION: YES the key is constrainable at its schema — BUT the Zod rejection's OWN issue path CONTAINS the rejected key, so constraining it ADDS a producer of terminator-bearing paths into the audit surface the cut protects.** ⇒ **the constraint makes the cut MORE load-bearing, not less; schema governs the CANDIDATE surface, the cut governs the AUDIT surface, and `L103` is correctly applied.** ⛔ **AND IT SURFACED A CROSS-TASK HAZARD NOW ON `### 24.104` AS A PRECONDITION: ajv puts a `propertyNames` violation's key in `params` — which `24.104` proposes to RESTORE — while Zod carries it in the path.** ***The two validator surfaces disagree and only one is safe by accident.***
