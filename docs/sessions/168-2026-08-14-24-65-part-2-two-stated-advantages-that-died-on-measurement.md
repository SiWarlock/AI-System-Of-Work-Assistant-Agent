...
**Successor:** `174-2026-08-14-24-68-the-remedy-that-could-not-be-built.md` (this area's next; `169`–`173` are worker's and knowledge's — chains run per-AREA, see the note in `161`) ⚠ *renumbered from `173`, which collided with worker's doc of the same number — see that file's "numbering collision" section.*
