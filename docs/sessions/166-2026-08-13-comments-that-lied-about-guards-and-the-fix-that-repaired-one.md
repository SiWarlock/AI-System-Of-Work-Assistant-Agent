...
## ⚠ ERRATUM 2026-08-13 — `ad0224f1`'s commit message is INCOMPLETE, and the omission is the slice's best finding

**Not false — incomplete.** The orchestrator's commit message was written against the Step-9 diff, and a **fifth correction post-dated it**. Recorded here rather than by rewriting history: `ad0224f1` is already cited above and in `#56`'s task metadata, an orchestrator commit sits on top of it, and rewriting would manufacture exactly the dangling-citation class this session existed to fix (`### 24.66` — dated erratum in place, never a rewrite).

**What the message omits (the change itself IS in `ad0224f1`; only its description was missing):** axis 1 of the dispatch addendum — *a correction owes a replacement* — found a fifth correction inside the already-finished diff. The old `buildActivities.ts` block also said *"do not read the absence of a behavioural test as an oversight"*, and that sentence had been **struck without replacement**. The claim did not become false: it became **TRUE FOR A DIFFERENT REASON**. Absence is now caught by the type system (`TS2741`); what no test can reach is whether the supplied **STRING** is correct. Restored with the new reason and a pointer to the block carrying it in full.

⭐⭐ **The reusable form: *the claim didn't become false, it became true-for-a-different-reason — which is the case most likely to get STRUCK rather than REWRITTEN.*** ⛔ A false claim announces itself; a still-true claim whose **ground moved** reads as merely stale, so the cheap edit is deletion — and deletion silently discards a real guarantee nobody re-derives. `contracts L146`'s shape (*what was the wrong value incidentally preventing?*) aimed at prose instead of code, and a sharper instance, **because here the correction and the deletion look identical in a diff.**

⭐⭐ **Axis 2's discriminator, ruled and confirmed: an instruction whose imperative STRENGTHENS is safe to fix in place; one whose imperative INVERTS needs authorization.** `legacy-workspace.ts`'s *"do not delete"* went from advisory to load-bearing when step 3 made the const the sole supplier of a required parameter — only its time-binding was stale, so re-tensing is a comment fix. Contrast the desktop case, where a comment told a reader to **perform** an action the new rule forbids.

⛔ **And the observation that re-prices the whole slice: NONE of the three dispatched sites contained an instruction. The only instruction was in the FOURTH site I added.** ⇒ ***the axis-2 risk lived entirely in the part nobody had scoped*** — simultaneously the strongest argument for the addendum's guard and for expanding scope past the filing. **A guard aimed at a filed scope cannot cover what the filing missed.**
