...
## Option A — app-managed serve + unify transport (owner-picked, BUILT)

The owner chose Option A (over external-serve+runbook or pause) to make the agentic tools functional end-to-end. The DB-lock contention (a running `serve --http` vs the `gbrain call query` CLI, one PGlite connection) is resolved by the app owning ONE serve that BOTH the http retrieval + the agentic tools share (the shared DCR token provider — boot.ts:426 — already unifies them).

- **SS1** (`e029115`, `apps/worker/src/gbrainServeSupervisor.ts`, dual-reviewer clean, 9 tests) — `createGbrainServeSupervisor`: spawn `gbrain serve --http --enable-dcr --port <p>` → poll-until-ready (bounded, iteration-based) → bounded restart-on-crash → dispose. Deterministic core over injected spawn/probe/sleep; real seams (`createGbrainServeSpawner` child_process, `createGbrainServeProbe` fetch) integration-gated. Fail-closed, never-throws, redaction-safe (stdio ignored). Review fixes in-slice: derives `--port` from baseUrl + refuses non-loopback baseUrl; re-checks `disposing` after the awaited probe; call-once guard; no trailing sleep. ⚠ **BIND-INTERFACE RESIDUAL:** gbrain has NO `--host` flag (verified via the CLI), so the bind interface is gbrain's (loopback) default — unfixable from a flag; documented.
- **SS2** was ALREADY a boot flag (`copilotGbrainTransport:"http"`, boot.ts:442) — no new code.
- **SS3** (`7554086`, `apps/desktop/worker-host/index.ts` + a `gbrainServe` worker-barrel export, security-reviewed) — worker-host starts the supervisor before bootWorker; on ready sets `copilotGbrainHttpUrl` + `copilotGbrainTransport:"http"` (bootWorker UNCHANGED — already consumes them); disposes on shutdown + on boot-failure (M1 leak fix — never orphan a serve holding the port/DB-lock). **`MANAGE_GBRAIN_SERVE` ships DEFAULT OFF (M2):** spawning a DCR MCP server on every boot couldn't be validated this session, and the bind residual is real, so enabling is an OWNER runtime step AFTER verifying serve becomes ready + binds loopback (`lsof -iTCP:8899 -sTCP:LISTEN` → 127.0.0.1). On serve failure boot degrades gracefully (CLI retrieval + fail-closed tools) within a 10s bound.

**Honest limitation:** SS3 is imperative composition-root wiring that spawns a real process — NOT end-to-end validatable in this session without spawning gbrain serve (a real DB-lock side effect). It typechecks + degrades gracefully + is security-reviewed; the runtime validation (does serve actually come up + bind loopback) is the owner's enable step. Deferred robustness (review lows): SIGTERM→SIGKILL escalation, `uncaughtException` dispose hook, restart backoff + reset.
