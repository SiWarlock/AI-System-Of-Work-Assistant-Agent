...
> ⭐⭐ **2026-08-12 — THE BLANKET ARMING BLOCK WAS RELEASED BY THE OWNER (all four conditions discharged). ⛔ THIS RUNBOOK'S GATES ARE UNCHANGED.** **Every crossing below remains OWNER-GATED by its own `§ARM-*` ledger. NOTHING IS ARMED. NO CROSSING MAY EXECUTE WITHOUT EXPLICIT OWNER CONFIRMATION AT THE TIME.** ⭐ **The release returned the project to PER-CROSSING gating — it granted NO crossing.** ⛔ **Do not read "the block was released" as permission to cross any step here.**
>
> ⚠⚠ **READ BEFORE PLANNING ANY CROSSING — an OPEN RECOMMENDATION, deliberately NOT a blocker (the owner released without conditioning on it): the `24.6` audit's severity gradings (`docs/audits/001`–`004`) REST ON REACHABILITY QUALIFIERS, and that class of claim was FALSIFIED THREE TIMES IN ONE SESSION by three competent people working BY HAND** (contracts **`L141`**; the failures are **biased toward over-claiming reachability**). **`24.29`'s repo-wide mechanical census exists and has NOT been pointed at them.** ⇒ ⭐ **RE-DERIVE THE REACHABILITY QUALIFIER FOR YOUR CROSSING'S FINDINGS BEFORE RELYING ON THEIR SEVERITY.** ⚠ **A finding graded *"dormant, therefore not a live breach"* is exactly the shape that was wrong three times.**
...
- **You have decided which trigger drives the write:** the proof-spine **propose activity** (`buildActivities.ts:436`, automation-driven, e.g. meeting-close) OR the **§9.8 approved-dispatch command** (`buildActivities.ts:521`, owner clicks "approve" on a pending card). ⛔ UPDATED 2026-09-22: the approval→write bridge is now BUILT — boot binds `externalApprovalDispatch.ts`, which sends an approved card's saved action through the Tool Gateway in the card's workspace, and refuses without writing while no real sender is armed. Only Linear has a real sender (`SOW_LINEAR_WRITES`); every other vendor is refused (`target_not_armed`).
...
4. ✅ **DONE 2026-09-22 (Linear slice 3+4):** the §9.8 approved-dispatch path is wired — `apps/worker/src/composition/externalApprovalDispatch.ts`, bound in `bootWorker` when the host passes no `dispatchApproval` override.
...
- **Approval path does not send even when approved.** Since 2026-09-22 this is by design when no real sender is armed: the dispatcher refuses as `writes_off` and records no receipt (owner decision — never a fabricated one). Fix: arm the sender (for Linear, `SOW_LINEAR_WRITES` plus a resolvable key). A card whose saved action is missing, or whose workspace or payload no longer matches, is also refused and reported in System Health.
