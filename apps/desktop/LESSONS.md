...
## <a id="23"></a>23. A fetched-on-demand PER-ROW detail needs the `PostureCell` shape — and the seq guard must invalidate on every SET REPLACEMENT, not only unmount, AND clear a stuck `loading` cell

**Date:** 2026-07-31. **Source slice:** 9.41 leg C (`3640c0e4`) — the Recent Activity audit-drill affordance. **Extends the `egress.tsx` pattern (L46's neighbourhood); the second and third clauses are what this slice ADDS to it.**

`egress.tsx:77-135` already established the shape for per-item async state: a `Record<id, Cell>` where `Cell` is a 3-state discriminated union (`loading` / `ready` / `unavailable`), a **monotonic per-item sequence guard** that drops a superseded resolution, fail-closed presentation, and invalidation of in-flight reads. Reuse it whenever a row fetches its own detail. **Two things did not transfer, and both were found by adversarial review rather than by copying:**

⭐ **(a) Invalidate on every SET REPLACEMENT, not only on unmount.** `egress.tsx`'s cleanup fires on unmount/re-key, which is enough when the list is stable. A feed that re-hydrates underneath the user is not: `hydrateScope` (`renderer/lib/live.ts:196-245`) **clears and re-fetches `recentChanges` wholesale on every scope change** and leaves it cleared under Global. Key separation alone does not save you — it covers A→B (a foreign resolution cannot key onto a rendered row, because `deriveChangeId` hashes `workspaceId` in) but **not A→B→back-to-A**, where the original row's own id reappears and a stale resolution can silently populate a row nobody re-activated this visit. ⚠ **And key separation is an invariant enforced in ANOTHER PACKAGE** (`apps/worker/src/api/projections/recentChanges.ts:62-65`) — desktop neither owns nor pins it, so per **L66** the pin must NAME it rather than rely on it silently.

⭐ **(b) An invalidated in-flight row must be CLEARED, not just ignored — otherwise it reads "loading" forever.** Found by the security reviewer, and it is the non-obvious half: the seq bump correctly stops the wrong paint, but the cell is still `{kind:"loading"}`, so the row shows *"Checking details…"* **permanently, with no retry**, if its context is ever revisited. The fix clears a stuck `loading` cell back to not-yet-activated while **retaining** `ready`/`unavailable` cells (an audit record is immutable, so a resolved summary stays valid). **A guard that prevents a wrong result and strands the UI has traded a visible error for an invisible one.**

⚠ **The dependency must be a DERIVED STABLE KEY, never the array reference.** With `[changes]` as the dep, every parent re-render bumps every seq and **no drill can ever complete** — and that failure is invisible to the obvious suite: the happy-path test does not re-render, and the superseded-resolution test *passes harder* when nothing ever paints ([L80](#80)'s "replace the gate with a constant DENY" aimed at an invalidator). **Pin it both ways** — that stale resolutions do NOT paint, and that a fresh one still DOES across an innocuous re-render. Key on a joined id string; here `encodeURIComponent` was **deliberately not** copied from `egress.tsx` because `changeId` is sha256 hex and cannot contain the delimiter — **the reason is stated in-code**, since an unexplained divergence from a precedent reads as an oversight.

`pin: apps/desktop/test-dom/today-recent-activity.test.tsx — a_superseded_resolution_never_paints_after_a_scope_change + a_drill_completes_across_an_innocuous_re_render (mutation-verified by reverting the dep to the raw array) + a_row_still_loading_when_the_scope_changes_is_not_stuck_forever`
