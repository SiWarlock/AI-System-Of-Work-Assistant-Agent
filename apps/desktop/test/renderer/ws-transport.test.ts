...
function fakeClient(): {
  client: unknown;
  calls: { input: unknown; handlers: { onData: (i: unknown) => void } }[];
  unsubs: () => number;
} {
  const calls: { input: unknown; handlers: { onData: (i: unknown) => void } }[] = [];
  let unsubscribed = 0;
  const client = {
    stream: {
      onEvent: {
        subscribe: (input: unknown, handlers: { onData: (i: unknown) => void }) => {
