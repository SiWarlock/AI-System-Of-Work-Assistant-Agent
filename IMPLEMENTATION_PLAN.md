...
- **⭐ Wave-2 fan-out SEALED 2026-07-25 (pre-compaction recovery anchor, round `<this commit>`).** 14 slices / 6 areas committed + pushed: **13.3 [x]** (a local-embed + b recall@10 harness, retrieval) · **13.17** re-ranker logic · **21.1/2-logic** + **13.13** (ProviderId perplexity/xai + RES-1 research provider/aggregator, dormant) · **12.16**/**12.18** eval legs · **13.8a** EntityResolver (ARC-4 keystone start) · **9.12r+A1** (9.12 renderer-affordance done) · **13.16** UiSafeTaskRollup type · **⚠SAFETY 9.10-A** store-backed egress posture (rule-5 fail-open closed on prod, personal `[claude]` seed, employer fail-closed). All packages green; anti-corruption count-pin 19→20 reconciled. 6 implementers HOLDING (lead compacting).
- **⚠ OWNER-PENDING (via lead, post-compaction):** **9.10-B** employer-open audited ack — employer cloud-egress OWNER-APPROVED in principle (§ARM-18) but gated on the owner confirming **login=company-subscription** at the flip · the 9.10-A dormant 4th-site DENY→ALLOW + cloudCopilotPosture deletion · **21.3** write-enum coverage (#27).
- **⚠ DEFERRED to the next round (task #31) — the seal prioritized the code+plan anchor:** this round's **LESSONS** (~12: logic-in-package/wire-at-boot · egress-veto-first research provider · ground-before-write resolver · recall-bar · closed-target renderer · …) + **ARCHITECTURE** arch-notes (13.13 §7/AppA · 13.8a §6 · 9.10-A §5/§16) + CLAUDE.md cross-doc rows + EVALUATION_CRITERIA recall value (fused 1.00) + the runbook login-pin are NOT yet written — the post-compaction successor banks them from #31. **NEXT:** ARC-4 13.8b→g · the worker composition-root bindings (21.1/2 #29 · 9.9a #28→9.9b #25 · 13.17 retrieval) · 13.14 /research · evalsec 12.x/Phase-19 · provint 21.10/25.x.
...
3. **9.10 Employer-Work egress-ack arc (owner-approved D1=A, ⚠SAFETY rule-5)** — A (store-backed posture) DONE this round (`40414dd1`). REMAINING: **B** audited acknowledge/revoke command (employer-open — Step-9 → LEAD, owner-gated on **login=company-subscription**, §ARM-18) · **C** desktop workspace-settings egress surface · **D** audit-link (co-designs 9.5) · the 9.10-A dormant source-ingestion 4th-site (DENY→ALLOW) + cloudCopilotPosture deletion follow-ups. Design: `docs/design/9.10-egress-ack-surface-design.md`. *(origin: 2026-07-25 9.10 design)*
...
**⭐ EMPLOYER cloud-egress OWNER-APPROVED IN PRINCIPLE (2026-07-25, via lead) — NOT yet open.** The owner consciously accepts Employer-Work cloud egress (rule-5 acknowledged), GOVERNED by using the **COMPANY's Claude subscription** (company-sanctioned → resolves the confidentiality/policy concern). The open is NOT a silent seed — it goes through **9.10-B's deliberate two-step-confirm + AUDITED acknowledge command** (the D1=A control; creates the audit trail). ⛔ **LOAD-BEARING PRECONDITION (gates the employer flip — pin also in `docs/runbooks/phase-18-subscription-enable-decision.md`):** ONE `claude` login today (no per-workspace subscription routing). "Company-sanctioned" holds ONLY if the active `claude` login IS the company subscription when employer content egresses ⇒ the employer-open ARMING is gated on the owner confirming **login=company-subscription at flip time** (routed through the lead); until then employer stays **FAIL-CLOSED**. Building the 9.10-B control is fine; the real employer egress is the gated flip. PERSONAL path = 9.10-A Option A + seed personal (personal_business/personal_life) allowlist=[claude] (single-source store, no regression). Don't assume employer stays fail-closed once 9.10-B + the flip land.
...
- [~] PARTIAL · `67c8f39a` · 2026-07-25 — worker-API session-token/Origin auth leg (drives the exported interceptor/originAllowlist/handshake boundary; anti-DNS-rebind spoof vectors [origin-raw-match + host-authority-isolation] + handshake token-source discipline). remaining: the Electron security/IPC leg (deferred — desktop, 9.14 overlap).
...
**State:** OPEN · **Kind:** build · **Spec:** §6 (KN-11) · **Depends:** 13.8a, P4 (KnowledgeWriter `applyLink`) · **Blocks:** 13.8c
