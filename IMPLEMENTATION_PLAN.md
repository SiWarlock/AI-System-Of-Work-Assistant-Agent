...
- **⭐ Remaining-build-order parallel execution — WAVE 1 SEALED 2026-07-25 (round `<this commit>`).** ARC 2 frozen-contract bottleneck **13.15** (Task+priority+TaskRepository + 4 operational FailureClass members, `54b052a7`, repo-wide typecheck 20/20) · ARC 0 integrity sweep **7.19** retention-prune dormant (`addd616f`) + **11.2** schema-compat refusal wired (`5ad722a8`) · ARC 1 desktop **9.14** adversarial specs (`25029a76`). Suites green (contracts 737 · db 447 · workflows 570 · worker 1898 · desktop 403). spec-lint `[~]`-task-check fixed (scripts/spec-lint.sh:92; scaffolding-repo port = task #19).
- **IN FLIGHT (fresh-orch handoff):** contract **UiSafe-types** round (brief 167, Step-2.5 APPROVED, GREEN pending — unblocks desktop 9.9/9.8/9.5) · worker **9.10-A** egress-posture store-backed read (brief 166, ⚠SAFETY rule-5, queued task #20) → 9.10-B. Desktop idle pending UiSafe-types.
- **NEXT (fresh orch):** land UiSafe #21 Step-9 + author 9.10-B/C/D; fan out Wave 2 — knowledge (ARC 3 retrieval 13.3/13.17 → ARC 4 §13.8 keystone + ARC-6 Phase-19 knowledge legs) · prov-int (ARC 5 dormant machinery) · eval-security (ARC 6 suites). Roster + handoff → session doc 111.
...
2. **Phase-9 completion → `/phase-exit 9`** — DONE this round: 9.14 renderer-isolation/redaction specs (`25029a76`). REMAINING before the gate: 9.9 Calendar · 9.10 egress-ack arc (item 3) · 9.21 idempotent scaffold repair · 9.6 real Copilot. Gated on the UiSafe-types round (#21, brief 167) + worker's `createWorkspace` partial-result shape (9.21). *(origin: Phase-9 census; no gate ever run)*
3. **9.10 Employer-Work egress-ack arc (owner-approved D1=A, ⚠SAFETY rule-5)** — Brief A (166, store-backed posture read, worker #20 queued) → B (acknowledge/revoke command, server two-step-confirm) → C (desktop workspace-settings surface) → D (audit-link, co-designs w/ 9.5). Own commits, security-reviewer=invariant, Step-9 re-routes to lead. Design: `docs/design/9.10-egress-ack-surface-design.md`. *(origin: 2026-07-25 9.10 design)*
...
**State:** OPEN · **Kind:** build · **Spec:** §6 (KN-10), §5 (WS-8) · **Depends:** P4 (GBrain adapter/index), 13.3 (retrieval) · **Blocks:** 13.8c, 13.8d, 13.14
