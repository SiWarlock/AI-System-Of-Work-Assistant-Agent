...
- **⭐ Wave-2 fan-out ROUND SEAL 2026-07-26 (post account-limit reset ~02:50 UTC).** A large parallel round across 6 areas. **⭐ ARC-4 living-vault KEYSTONE chain 13.8a→b→c→d COMPLETE:** planner `461a0186` (13.8c SENSE→REASON→EFFECT, tiered-autonomy + symmetric fail-closed @user confinement) + ingest-rewrite `04e01eed`+`3d2d24f9` (13.8d "vault rewrites itself" + structural-file parity + KW create/patch marker-neutralization) + eval `a34de8e1` (13.8c-eval). **§13.16 task-surfacing COMPLETE:** producer `0e6e1662` + renderer `985c1dda`. **§9.9 Calendar** `8b4e3537`+`f9d86536`. **21.1/2 binding** `ed9faa26` · **12.20 doctor-eval** `6b38f022` · **13.2a web transport** `3c501687`. **§21.3 read/write-symmetry CLOSED (#27):** todoist `6facc356` + telegram `15a90ed4` READ connectors (dormant, ING-7) + the pathAuth template mode; gmail/granola no-write ATTESTED (owner-decided). Per-package suites green.
- **⚠ CYCLE IN PROGRESS + IN-FLIGHT:** worker **#53 9.10-B egress-revoke** (rule-5, brief 197, Step-2.5 APPROVED, Step-9→LEAD) mid-slice at seal. Team cycling on the limit-reset: ORCH sealed here + drains; evalsec/provint/worker → /session-end → fresh sessions (lead shutdowns+respawns).
- **⚠ FRESH ORCH — COMPLETE THE #31 ROUTING + dispatch.** This seal banked the briefs (189–198) + this snapshot + Log; the **tick/lesson/arch hot-routing rolls to you — the full inventory is in task #31** (pending ticks 9.9/21.1-2/12.20/13.16/13.8c-d/13.2a/21.3; ~9 lesson candidates; arch notes §6 KN-10 stance+confinement / §6 KN-12 / §8 connectors / §5-16 egress-revoke; #27 attestations→24.5 runbook). NEXT DISPATCH: 13.8f meeting-path + 13.8g attendee (keystone) · 13.8d worker-binding · #54 noteSlug re-point · #55 evalsec ripples (guard-L12+count-pin+egressCommand — @sow/evals gate RED on HEAD until done) · 9.10-C desktop egress-settings (after #53). push owner-run (many ahead of origin).
...
1. **The SPINE arc** — connector→ingestion→content→gbrain end-to-end (owner-sequenced post-connector-chain target; folds the dormant extractor real-transports as sub-rounds, each owner-confirmed to bind — §ARM-23). Includes 13.10c gmail-source content hydration **AND the Granola transcript hydration + meeting-path living-vault synthesis (23.8 / 13.8f/g — the meeting leg produces the same entity-grounded person/project updates the source leg does)**. *(origin: 2026-07-15 R8 close, owner breadth-then-spine decision; amended 2026-07-24 Granola-gap integration)*
2. **Phase-9 completion → `/phase-exit 9`** — DONE this round: 9.12r+A1 (renderer open/reveal → 9.12 done), 9.10-A egress posture. REMAINING before the gate: 9.9 Calendar (split 9.9a #28 producer → 9.9b #25 renderer) · 9.10-B/C/D egress-ack + audit-link (item 3) · **9.11 first-run gating (⚠ FINDING: `apps/desktop/main/first-run.ts` EXISTS + brief 158 — plan marks it "absent"; RECONCILE — likely already built)** · 9.6 real Copilot. *(origin: Phase-9 census; no gate ever run)*
3. **9.10 Employer-Work egress-ack arc (⚠SAFETY rule-5)** — A store-backed posture (`40414dd1`) + **the employer-egress FLIP EXECUTED** (`bcde3d61`, owner-authorized rule-5 default-seed `[claude]`, §ARM-18 EXECUTED — employer cloud egress now OPEN-by-default-seed). REMAINING: **B** re-scoped to egress-status VISIBILITY + REVOKE surface (no longer the opener) · **C** desktop workspace-settings egress surface · **D** audit-link (co-designs 9.5) · the 9.10-A dormant source-ingestion 4th-site + cloudCopilotPosture deletion follow-ups · the login-switch residual → subscription-split end-state. *(origin: 2026-07-25 9.10 design; flip 2026-07-25)*
4. **`selection_failed`→`db_unavailable` reclassification (7.19)** — now buildable (db_unavailable landed 13.15/`54b052a7`); retentionPrune.ts currently maps the store-read/enumeration fault to `write_through_failed` as a documented arch_gap interim. Small worker follow-up. *(origin: 2026-07-25 7.19)*
5. **spec-lint `[~]`-task-check fix → scaffolding repo (task #19, MANDATORY)** — the in-target one-liner (scripts/spec-lint.sh:92, accept `[~]`) needs porting to the scaffolding template so scaffold-upgrade doesn't clobber/regress it. *(origin: 2026-07-25 gate finding)*
...
**Files:** apps/desktop/renderer/surfaces/onboarding/ · renderer/App.tsx (onOnboarded partial-result handling)
...
- [ ] OPEN — decomposed into buildable sub-slices 13.8a–e below (each dormant; deterministic legs TDD, model legs eval-tested; the scheduled leg 13.8e arms via §19.13/§ARM-RESEARCH)
...
**State:** OPEN · **Kind:** build · **Spec:** §9 workflow-1 (L297 — KnowledgeWriter meeting/project/person/decision/daily targets), §6 (KN-10/11/12) · **Depends:** 13.8a (EntityResolver), 13.8b (LinkHealer), 13.8c (planner) · **Blocks:** 23.8
...
**State:** OPEN · **Kind:** build · **Spec:** §9 workflow-1 (L297 — "correlate … attendees …" + KnowledgeWriter person target), §6 (KN-11 entity convergence) · **Depends:** 13.8a (EntityResolver), 13.8f · **Blocks:** 23.8
