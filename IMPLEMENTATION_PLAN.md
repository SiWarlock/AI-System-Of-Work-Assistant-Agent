...
- **⭐ §ARM-18 auto-ingest ENABLE crossing SEALED LIVE 2026-07-24** — the maiden auto-ingest-armed real subscription extraction succeeded over a throwaway test vault. Gates 2→6 all passed; **Step A (direct-extraction) real run = PASS**: note `sources/ws-gate6-testvault/5090325d…md`, **$0.054601** metered (< $1.50 cap), `apiKeyUnset`, egress `claude-agent-sdk` only, REQ-F-017 `TBD` held, ING-7, and the **gate-3 control-var unknown CLOSED live** (childEnv = 8-key allowlist + SDK re-injected `CLAUDE_CODE_SDK_*` atop it, `runOk:true`). Auto-mode classifier blocked the worker → owner ran the command himself (session-098 fallback, no bypass). Step B (in-app `SOW_INGEST_WATCH` watcher auto-fire) = documented owner-run follow-up (runbook `phase-18-10-auto-ingest-enable-readiness.md`). No new committed code (throwaway harness removed). Session doc 106.
- **⛔ NEXT — OWNER-DIRECTED new scope: full OSB PARITY amendment integration.** A grounded amendment proposal (auto-linking docs to related entries · auto-updating related entities [people/projects] on ingest · the `/research`+`/research-deep` web-research flow) lands at `docs/planning/osb-parity-amendment-proposal.md`; after lead review it routes to the orchestrator to integrate into canonical `ARCHITECTURE.md` + `IMPLEMENTATION_PLAN.md` as new phases/tasks. **Do NOT start until the lead routes it.** Meanwhile the Carry-forward working set stands (SPINE · 7.19 retention · Phase-9 `/phase-exit` · 11.2 · FailureClass enum · /design-review · ESLint). Suite 6413 green.
