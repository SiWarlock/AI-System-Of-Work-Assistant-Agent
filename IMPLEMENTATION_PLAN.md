...
- **§ARM-18 CHECKPOINT-1 pre-first-armed-run hardening round SEALED 2026-07-20** — 3 dormant slices: 18.35 `ee1d8a34` (reachability OFF-lock — ENABLE is now a config flip, not a code patch) · 18.36 `8c9763ff` (settings-level key-injection guard) · 18.37 `afb98684` (shadow set 13→30 + `gcpAuthRefresh`; 5th-switch rule-5 fail-open caught+fixed). Session doc 104 · briefs 149–151 · worker L70/71/72. Suite 6387 green.
- **Next target — IN PROGRESS (owner-authorized):** the ACTUAL auto-ingest ENABLE over a FRESH THROWAWAY TEST vault (benign content; real `~/Obsidian/brain` OUT of scope). Drive per §ARM-18 CHECKPOINT-1: ⛔ HARD repo-source shadow-guard re-ground + live-SDK-docs re-ground + go/no-go `SOW_L64_DRYRUN` `$0` + op prereqs (claude login / keychain Allow / temporal) → flip (`SOW_SUBSCRIPTION_ARM=1` + `SOW_EGRESS_ALLOWED_PROCESSORS=claude-agent-sdk` + `SOW_MANAGE_TEMPORAL=true` + `SOW_VAULT_ROOT=<test>` + `SOW_SUBSCRIPTION_REACHABILITY_LIVE=1`) → verify ONE real e2e (real subscription → real KnowledgeWriter note in the test vault). worker-impl-driven; flip owner-authorized.
