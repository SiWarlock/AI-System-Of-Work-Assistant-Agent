...
- **⭐ Phase-9 "make the daily briefing real" — round 1 LANDED 2026-07-24.** The Recent Changes surface now renders REAL ingest data: **9.15** (`8a35cc2e`) real recent-changes read-model producer (scoped `audit.query` → `projectRecentChanges` → `readModels.put` via a bounded fail-safe `sourceCommit` post-commit trigger; reachability CONFIRMED live — real commit → real `recent_changes` row) + **9.12** (`80d387c6`) path-scoped open-in-vault (realpath-contained `guardVaultPath`). Both security 0C/0H/0M; worker L76 + desktop L16. Suites green (worker 6435 · desktop 376). Session docs 108 (worker) · 109 (desktop).
- **NEXT — remaining Phase-9 legs (fresh cycled trio):** **WORKER** = the daily-brief / dashboard / task-rollup producer legs (mirror the 9.15 bounded fail-safe `sourceCommit` seam — populate the remaining Today read-model keys from real ingested notes + extracted tasks, WS-8 sanitized) + the ingestion-inbox park-sink binding (`buildActivities.ts:426` no-op stub → the real `createIngestionInboxProjectionPort`). **DESKTOP** = Today-live wiring for the remaining STATIC sections (daily-brief / nav-counts / egress-pill — each needs its worker producer first) + 9.9 Calendar / 9.10 egress-ack + audit-link / 9.11 first-run gating + idempotent repair. **THEN** `/phase-exit 9` (desktop track's first formal gate) + the owed LIVE `/design-review` (Carry-forward #6). Audit.query recency arch_gap (→ task 2.9) + refresh-fault→HealthItem are in Residuals(9).
- **Prior milestones (sealed, nothing armed):** §ARM-18 auto-ingest ENABLE crossing SEALED LIVE (`30ef70e0`, subscription extraction over a throwaway test vault; Step B watcher-auto-fire = owner-run follow-up, runbook §6) · OSB-PARITY amendment INTEGRATED (`a4242735`, planning-only/dormant — KN-10/11/12 + RES-1/2 + §19.13/Phase 26/§ARM-RESEARCH + 13.8a–e/13.13/13.14). Standing Carry-forward: SPINE · 7.19 retention · 11.2 · FailureClass enum · ESLint.
