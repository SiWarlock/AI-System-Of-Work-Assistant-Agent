...
- **⭐ Phase-9 "make the daily briefing real" — round 2 LANDED 2026-07-24 (seal `<this commit>`).** 5 slices, both tracks: **9.18** (`2ce23ef1`) ⚡ startup-bug fix — Electron main bundles `@sow/contracts` (was require()-ing raw `.ts` → SyntaxError, regression from 9.12); **`./dev.sh` now BOOTS, owner unblocked**. **9.16** (`d44a1f24`) ingestion-inbox park-sink producer bound at the composition root (park upserts / disposition removes, fail-SAFE, WS-8). **9.17** (`2405c39f`) authoritative durable first-run gate (main-owned marker + backfill; completes 9.11). **9.19** (`0c8aa450`) DEV-ONLY `SOW_DEMO_SEED` demo-seed (full Global Today read-model, read-model-only). **9.20** (`cc811286`) Today-live — deterministic model-free daily-brief + honest-empty schedule. **Owner can `SOW_DEMO_SEED=1 ./dev.sh` → a genuinely populated Today, zero model calls (live e2e verified).** Suites green (worker 1895 · desktop 396); all reviews CLEAN/low.
- **NEXT — owner-feedback-steered (both tracks HELD for dogfooding):** 9.9 Calendar · 9.10 egress-ack (⚠ SAFETY-SENSITIVE rule-5-adjacent — lead-review-gated BEFORE dispatch) · 9.14 renderer-isolation/redaction specs · 9.6 real Copilot · the real dashboard/task-rollup producer (empty-until-real-data) → then `/phase-exit 9` (desktop track's first formal gate) + the owed LIVE `/design-review`.
- **Queued MAJOR arc (owner-confirmed, post-Phase-9) — now INTEGRATED as canonical tasks (2026-07-24 planning round, anchors VERIFIED against real code):** §13.8 living-vault synthesis (13.8a→e keystone) EXTENDED — **13.8f** meeting-path synthesis rewrite (buildOutputs.ts:242-243 empty link/frontmatter) · **13.8g** attendee→person fan-out (ARCH §9 W1 L297 unimplemented-SPEC gap, confirmed) · **23.8** Granola SPINE (OWNER-GATED §ARM-23) + Carry-forward-#1 amended · plus the "highest-priority tasks" fix — **13.15** typed Task model + `priority` (FROZEN-CONTRACT) · **13.16** priority/due-ranked task read-model · **13.17** retrieval re-ranker (§9.6-real). All DORMANT/PLANNED, no code build; owner steers BUILD order (§13.8 arc vs Phase-9 completion). Standing Carry-forward: SPINE · 7.19 retention · 11.2 · FailureClass enum · ESLint.
...
3. **Phase-9 completion → `/phase-exit 9`** — DONE this round: 9.11 (first-run, 9.17) · 9.12 (open-in-vault) · 9.15/9.16 (producers) · 9.19 (demo-seed) · 9.20 (Today-live brief). REMAINING before the gate: 9.9 Calendar · 9.10 egress-ack (lead-review-gated) · 9.14 renderer-isolation/redaction specs · 9.6 real Copilot · 9.21 idempotent scaffold repair. Then run the phase's first formal exit gate. *(origin: Phase-9 census; no gate ever run)*
