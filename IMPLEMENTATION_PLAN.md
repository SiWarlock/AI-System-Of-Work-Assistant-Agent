...
- **§ARM-18 CHECKPOINT-1 GATE-1 rule-5 completeness SEALED 2026-07-24** — 3 dormant slices: 18.38 `8ac03a48` (shadow denylist 30→81, DEFENSE-IN-DEPTH belt) · 18.40 `46198ace` (ENV surface complete-by-construction — minimal allowlisted child env at both `query()` spawn sites; the denylist is unwinnable — `CLAUDE_ENV_FILE` bypasses the process.env scan) · 18.39-B `e11f0a6d` (SETTINGS-FILE surface complete-by-construction — presence-degrade on any managed-tier settings + `settingSources:[]` disabling user/project). BOTH GATE-1 rule-5 legs code-complete by-construction; the per-version re-verify preconditions RETIRED. Session doc 105 · briefs 152–154 · worker Lessons 73/74. Suite 6413 green.
- **⛔ NEXT — the crossing resumes at GATE-1 CLEAR (fresh orch+worker pair; flip owner-gated):** gate-2 (live Agent-SDK-docs re-ground) → gate-3 (**the $0 `SOW_L64_DRYRUN` — SDK `CLAUDE_CODE_SDK_*` control-var survival + note-produced go/no-go; NOT yet run**) → gate-4 (op-prereqs: **no managed settings on the armed host = STOP+surface** + claude-login + `ANTHROPIC_API_KEY` UNSET + temporal) → gate-5 (owner-gated flip: `SOW_SUBSCRIPTION_ARM=1` + `SOW_EGRESS_ALLOWED_PROCESSORS=claude-agent-sdk` + `SOW_MANAGE_TEMPORAL=true` + `SOW_VAULT_ROOT=<fresh benign test vault>` + `SOW_SUBSCRIPTION_REACHABILITY_LIVE=1`) → gate-6 (one real e2e → real KW note; real `~/Obsidian/brain` OUT). §ARM-18 ledger has the detail.
