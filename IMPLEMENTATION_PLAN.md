...
- **⭐ Wave-2 — 2nd seal 2026-07-25 (round `<this commit>`).** Since the recovery seal `53231188` (the first 14 slices: 13.3/13.17/21.1-2-logic/13.13/12.16/12.18/13.8a/9.12r+A1/13.16/9.10-A): **⛔ 9.10 employer-egress FLIP EXECUTED** (`bcde3d61`, owner-authorized rule-5 default-seed employer_work=`[claude]`, scoped/non-blanket [non-claude still denies], preserve-fault, §ARM-18 EXECUTED) · **21.10-core** credential seam (`e023f682`, dormant) · **13.13r** research egress-leakage eval (`1aaa575a`). All committed, suites green. **Employer cloud egress is now OPEN-by-default-seed for `[claude]`** (company-sanctioned login=company; login-switch residual + subscription-split queued, §ARM-18).
- **⚠ IN-FLIGHT for the FRESH ORCH (orch-only cycle — the 5 impls keep running):** knowledge **#40 13.8b** LinkHealer (GREEN, Step-9 → fresh orch ships) · worker-implementer **#28 9.9a** calendar producer (RED→GREEN, brief 186, Step-2.5 → fresh orch; unblocks desktop 9.9b #25). provint/evalsec/desktop HOLDING for the fresh orch. NOTE: worker-implementer-2 was cycled on a false-alarm shutdown → **worker-implementer** is the live successor.
- **⚠ FRESH-ORCH close-out DEBT (task #31 metadata = the full checklist; bank next round):** a subagent is banking this seal's LESSONS (#24–37) + ARCHITECTURE arch-notes (§7 ProviderId · §6 EntityResolver/LinkHealer/RRF · §5/§16 egress-single-source + employer-flip) + CLAUDE.md cross-doc + EVALUATION_CRITERIA (recall fused 1.00) + runbook login-pin — **VERIFY those landed at close**; STILL owed in #31: §ARM-RESEARCH (13.13-provider) + §ARM-21 (21.10 arm-together + SecretsPort-triplication) residuals · the 9.11 first-run drift reconcile · the 12.18 DoD-pointer. **NEXT:** ARC-4 13.8c planner→d/f/g · worker bindings (9.9a→9.9b · 21.1/2 #29 · 13.17) · 13.14 /research · 21.3 (#27, owner) · **push (owner-blocked: `git push origin main`)**.
...
- [ ] OPEN — split by territory (lead ruling): 9.9a worker read-model producer (calendarProjection + query.calendar, task #28, pending) → 9.9b renderer Calendar surface (task #25, blocked on 9.9a). Neither built yet; the UiSafeSchedule contract type landed (#21 `28dd42ba`).
