...
- **⭐ Wave-2 seals 2026-07-25.** 2nd seal `96f7d8ab` (employer-egress rule-5 FLIP `bcde3d61` [owner-authorized default-seed employer_work=`[claude]`, scoped/preserve-fault, §ARM-18 EXECUTED] · 21.10-core `e023f682` · 13.13r `1aaa575a`; employer cloud egress now OPEN-by-default-seed for `[claude]`, login=company). **3rd seal `<this commit>` (HARD-STOP orch-only cycle):** respawned orch shipped **#28 9.9a** calendar producer `8b4e3537` (dormant/empty-until-wired, query.calendar mounted; Step-9 done + hot-routed), dispatched #29 + #25, hit HARD-STOP (96%) → sealed + cycled. Prior in-flight now SHIPPED: #40 13.8b `262df7b8` · #28 `8b4e3537`. Suites green.
- **⚠ IN-FLIGHT for the FRESH ORCH (impls keep running):** worker **#29 21.1/2 composition-root binding** (brief 187 `@b84826f8`, Step-2.5 APPROVED — both dispatch sites via `dispatchRouted`, unrouted-sentinel fail-closed, dormant/byte-equiv) · desktop **#25 9.9b Calendar renderer** (brief 188 `@0b280c6b`, Step-2.5 APPROVED, GLOBAL cold-load slice). BOTH Step-9 → fresh orch. ⚠ desktop green needs a fresh `pnpm build:sow` (stale gitignored worker dist).
- **⚠ FRESH-ORCH FIRST DISPATCHES + owed debt.** Idle areas (no in_progress task): ⭐ knowledge **13.8c** planner→d/f/g keystone + **13.16** rollup · provint **ARC-5** (13.2 / 25.x) · evalsec **ARC-6** (19.x / 12.x) · contract free. **STILL-OWED #31** (LESSONS #24–37 + arch-notes banked `96f7d8ab`): §ARM-RESEARCH (13.13-provider) + §ARM-21 residuals · 9.11 first-run reconcile · 12.18 DoD-pointer · §9.9 producer-binding safety follow-ups (rule-7 cause-redaction + genericReason via GCL Gate). **push owner-blocked** (`git push origin main`; 7 ahead).
...
**State:** OPEN · **Kind:** build + eval-tested · **Spec:** §6 (KN-10), REQ-F-017 · **Depends:** 13.8a, 13.8b, 13.3, P5 (ModelProviderPort + egress veto), P4 (KnowledgeWriter) · **Blocks:** 13.8e, 13.14
...
**State:** OPEN · **Kind:** build · **Spec:** §6 (KN-10/KN-12), REQ-F-021 · **Depends:** 13.8c, P4, P7 (ingestionTriage) · **Blocks:** 13.8e
