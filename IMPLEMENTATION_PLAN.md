...
- [ ] OPEN — split by territory (lead ruling): 9.9a worker read-model producer **SHIPPED** (calendarProjection + query.calendar mounted, dormant/empty-until-wired; brief 186; task #28; `8b4e3537`) → 9.9b renderer Calendar surface (task #25, IN FLIGHT, brief 188). The UiSafeSchedule contract type landed (#21 `28dd42ba`).
...
- [~] PARTIAL · `41e0dcca` · 2026-07-25 — the `UiSafeTaskRollup` contract type (contract-first, pre-ranked ordered list, WS-8 no-cross-blend/no workspaceId, priority-unset-representable [no-inference], `.strict()`/`.max(200)`). remaining: the worker task-rollup producer (WS-8 fail-SAFE, ranks by priority/dueDate) + the renderer Today/Copilot "highest-priority tasks" surface.
...
- [~] PARTIAL · `07145feb` · 2026-07-25 — the routing registry LOGIC (packages/integrations pure module: exhaustive `Record<TargetSystem,adapter>` + `Object.hasOwn` prototype-safe selector → `unregistered_target` + fail-closed sentinel + `dispatchRouted`). remaining: the worker composition-root binding (backends.ts/buildActivities.ts, task #29 — logic-in-package / wire-at-boot pattern). Closes G15 logic.
...
- [~] PARTIAL · `07145feb` · 2026-07-25 — the 7 vendor factories wired into `buildWriteAdapterRegistry` over the shared stub (bundled with 21.1-logic). remaining: the worker composition-root binding (task #29). Closes G16 logic.
