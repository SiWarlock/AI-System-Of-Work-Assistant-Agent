...
**(A) The SSRF predicate is the OUTBOUND INVERSE of the loopback guard.** gbrain's transport requires LOOPBACK (`isLoopbackEndpoint`); a connector requires an ALLOWLISTED REMOTE host and must REJECT loopback (SSRF-to-local). `isAllowedRemoteEndpoint(endpoint, allowedHosts)` = https + exact whole-host allowlist + non-loopback, **composed once** from the vetted `extractHost`/`isLoopbackHost` (root CLAUDE.md Lesson 4 — a safety predicate lives once; never re-parse a URL, because the parse is where the `evil.com/@127.0.0.1` userinfo-spoof holes live and are already closed). The exact-host allowlist is the primary control; loopback-reject is defense-in-depth. A hostname allowlist alone CANNOT catch DNS-rebinding. **`isPrivateHost` is now IMPLEMENTED (16.3, `5ce1961d`): a DENYLIST that BEATS the allowlist** — layered into `isAllowedRemoteEndpoint` after the loopback reject, blocking RFC-1918/CGNAT(100.64/10)/link-local+cloud-metadata(169.254)/ULA/`.internal` AND non-canonical forms (inet_aton numeric IPv4, IPv6 hex-mapped/expanded, trailing-dot FQDN, whitespace) via a hand-written IPv6-hextet + numeric-IPv4 parser, fail-closed on malformed; a security-review found 1 high + 2 med + 2 low evasions, ALL fixed in-slice + RE-VERIFIED PASS (public IPv6 stays public, vendor allowlists unregressed). The **resolved-IP pinning** half remains a documented `arch_gap` residual — the injected real transport must re-run `isPrivateHost` on the DNS-RESOLVED IP (closing DNS-rebind + NAT64/6to4 + `0x`-label inet_aton), finalized when the real transport (which does the actual DNS resolution) binds at arming (Phase-23).
...
## <a id="7"></a>7. The real model `HttpTransport` is a THIN injected-`fetch` pass-through (no status classify, no throw swallow, no log sink) that OWNS its egress wire — `redirect:"manual"` so a cross-origin 3xx can't re-send `x-api-key`

**Date:** 2026-07-17.
**Source slice:** 18.18b / flip-wiring (providers) `67f6b87a` (brief 132).

`createRealModelHttpTransport({fetch?})` is the real fetch-based model-layer `HttpTransport` the owner injects into `config.providerTransport` at the Phase-18 crossing (default fetch: the Node global). It is a THIN pass-through BY CONTRACT: it does the HTTP round-trip and returns `{status, body}` VERBATIM — it NEVER classifies status (`executeCompletion → providerErrorFromStatus` owns status→`ProviderError`), NEVER swallows/wraps a network throw (`classifyTransportThrow` owns abort/timeout/error), and takes NO log sink (`RealHttpTransportDeps` has no `logSink` field — a compile-time rule-7 guarantee; the secret-bearing headers/body are never echoed). Mock-fetch tested, NOT enabled (zero production call-site; 18.18a injects it at the owner ENABLE — L11-waivered). It HARDENS the one wire it owns: (a) **`redirect:"manual"` at the send boundary for EVERY injected fetch** (not just the default) — undici does NOT strip a custom `x-api-key` on a cross-origin 3xx, and the redirect target is chosen INSIDE fetch AFTER url validation (invisible to the upstream egress veto), so a followed redirect would re-send the key to an attacker origin; `"manual"` returns the 3xx to the executor → `providerErrorFromStatus` → fail-closed (rule 7). This was a security-reviewer MEDIUM (the default global fetch inherits `redirect:"follow"`), fixed in-slice. (b) **Headers COPIED at the send boundary** (a spread copy, never the caller's live secret map — no injected fetch retains a reference to it). (c) **AbortSignal forwarded** (cooperative cancel on budget breach, §5.4). The pass-through must not swallow: a 500 returns `{status:500, body}` with no throw (the executor maps it), and a network throw propagates by the SAME reference (L15 non-vacuous).

**Rule:** a real effect transport at a trust boundary is a THIN injected-`fetch` pass-through — it returns `{status, body}` verbatim and does NOT classify status, swallow/wrap a throw, or take a log sink (the executor owns status→error mapping + throw classification + redaction; the ABSENT `logSink` field is a compile-time rule-7 guarantee) — but it OWNS the egress wire it drives: set `redirect:"manual"` for EVERY injected fetch (undici doesn't strip a custom credential header on a cross-origin 3xx, and the redirect target is chosen inside fetch after url validation ⇒ invisible to the upstream egress veto ⇒ a followed 3xx re-sends the key; `"manual"` fails closed), COPY the headers at the send boundary (never retain the caller's live secret map), and forward the AbortSignal. Mock-fetch test it (no real network) + keep it dormant/uninjected until arming (L11). `pin: real-http-transport.test.ts request_maps_verbatim_to_fetch + non_2xx_passes_through_no_throw + network_throw_propagates_unchanged + drives_fetch_with_redirect_manual_and_copied_headers + no_secret_in_observable_output + abort_signal_forwarded_to_fetch`.
