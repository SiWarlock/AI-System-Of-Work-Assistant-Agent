...
⭐ **NOT THE WHOLE SET — the PROJECT-WIDE ledger is `packages/contracts/LESSONS.md`** (index: `packages/contracts/LESSONS-INDEX.md` — the FULL 256-row table; `packages/contracts/CLAUDE.md` keeps only an always-on 20-rule core, moved 2026-08-29 for context cost), designated by root `CLAUDE.md`; cross-area rules are banked there, never here. ⛔ **Until 2026-07-31 this file pointed only DOWN at its own ledger and nothing pointed back UP** — so an implementer orienting off this doc (what the launch protocol tells you to load) had no path to the project-wide ledger at all. **Read both.**
...
**Lesson numbers are stable IDs** — once assigned, they don't change. New lessons get the next sequential number. `/session-end` proposes additions when it detects them; the user approves before the entry is written and its row is added to that area's `LESSONS-INDEX.md` **in the same commit** — the row is the copy that gets paraphrased into code comments, and letting it drift is a measured defect (`### 24.147`).
...
| [6](LESSONS.md#6) | 2026-07-15 | Untrusted-source content is admitted READ-ONLY at JOB ADMISSION (source-agnostic + fail-closed), NOT at the adapter | ING-7 read-only for untrusted-content connectors/extractors (web/podcast/youtube/file) is enforced at JOB ADMISSION (`admitJob` — broker step 1, gates on `trustLevel` fail-closed AND `admitsMutating`, source-type-AGNOSTIC ⇒ `!trusted && mutating ⇒ DENY`, no per-source bypass), NOT in the adapter (a pure emit-only mapper — nothing to strip; its header only documents the posture). VERIFY by tracing the real symbols (a brief's names may be approximate — here `admitJob`/`admitsMutating`/`isTrusted`, not the brief's `untrustedJob`/`MUTATING_POLICY`); PIN with tests naming the untrusted source types + failing against per-source-allowlist / truthy-trust / always-deny breaks. An untrusted-content-consumed-by-non-read-only-job gap is a safety Finding, not a local fix. |
| [7](LESSONS.md#7) | 2026-07-17 | Real model `HttpTransport` = a THIN injected-`fetch` pass-through that OWNS its egress wire (`redirect:"manual"`) | `createRealModelHttpTransport({fetch?})` (the real client the owner injects at the Phase-18 crossing) is a THIN pass-through — returns `{status, body}` VERBATIM, does NOT classify status / swallow-or-wrap a throw / take a log sink (the executor owns those; the ABSENT `logSink` field is a compile-time rule-7 guarantee) — but OWNS the egress wire it drives: `redirect:"manual"` for EVERY injected fetch (undici doesn't strip a custom `x-api-key` on a cross-origin 3xx, and the redirect target is chosen inside fetch AFTER url validation ⇒ invisible to the upstream egress veto ⇒ a followed 3xx re-sends the key; `"manual"` fails closed — a security-reviewer MEDIUM fixed in-slice), headers COPIED at the send boundary (never the caller's live secret map), AbortSignal forwarded; mock-fetch tested + dormant/uninjected until arming (L11; 18.18a injects it at ENABLE). |
| [8](LESSONS.md#8) | 2026-07-18 | Enforced dollar cap → subscription/runtime route ONLY via the SDK `maxBudgetUsd`; thread by PRESENCE not truthiness; the budget enforcer is the cap authority | For a runtime route the token-priced budget gate can't meter (no `provider` on the route — §19.5 Finding-F), the dollar cap reaches the model ONLY via the SDK-native `maxBudgetUsd` (`CompletionRequest.maxCostUsd`, which `complete()` forwards); assemble it at the single `buildExtractionCompletionRequest` chokepoint, thread the enforced cap by PRESENCE (`!== undefined` — a `$0` cap MUST carry; a truthy check drops it and fails OPEN, Lessons 54/55), rely on the budget enforcer's positive-finite guarantee (don't re-police — an omit-on-non-finite guard fails OPEN), keep the builder pure + total; a separate `DEFAULT_EXTRACTION_BETAS` keeps a later extraction/synthesis divergence off the Copilot path. `pin: extraction-completion-request.test.ts threads_a_zero_cap_by_presence_not_truthiness`. |
| [9](LESSONS.md#9) | 2026-07-18 | Subscription health probe = a pure/total fail-closed fakeable primitive; STRICT `=== true` on EVERY dimension (never false-green); whole fold in ONE try; closed-set code-only reason | The AND-locked-to-transport HEALTH source (L52) needs a genuine fail-closed probe, never a false-green: `probeClaudeSubscriptionHealth({checkReachable})` folds an INJECTED reachability check (no SDK/network at build, mirrors the client's lazy import) by STRICT `=== true` on every dimension (`loginPresent` AND `sdkReachable`) so absent/undefined/null/partial/truthy-coerced/throwing ⇒ unhealthy with a closed-set code-only reason (rule 7); the WHOLE fold (call+inspection) in ONE try so a hostile result-getter can't break totality (pin the null-result branch — happy tests miss the TypeError); a minimal verdict the worker maps to `HealthGateSources` (18.23). `pin: subscription-health-probe.test.ts null_result_folds_ambiguous + partial_object_missing_sdk_reachable_is_unreachable`. |
| [10](LESSONS.md#10) | 2026-07-18 | Real spend-free `checkReachable` = login PRESENCE (fs/keychain existence, never contents) + module RESOLVABILITY (`createRequire.resolve` ≠ import); REJECT `accountInfo()`/`query()` readiness; login PATH a dated fail-closed placeholder | The real input to the L9 probe must be SPEND-FREE (runs at CP2 before any gated run): fold two concrete primitives — login credential PRESENCE via fs/keychain EXISTENCE only (rule 7, `security find-generic-password -s` NO `-w`; path a dated fail-closed placeholder — wrong ⇒ loginPresent:false ⇒ HALT, the arm injects the live macOS Keychain-presence detector) + module RESOLVABILITY via `createRequire().resolve` (≠ import, requires a non-empty resolved path — a "didn't throw" check fails OPEN, L52); REJECT `accountInfo()`/`tokenSource`/`query()` (a live session spawns the CLI + can SPEND at CP2, L55/L56); strict `=== true` both dimensions (no false-green). `pin: subscription-reachability-probe.test.ts throw_folds_to_not_reachable + spend_free_delegates_only_to_injected_primitives` |
