...
function toOutbox(r: OutboxRow): OutboxEntry {
  return {
    ...
    payloadHash: r.payloadHash,
    approvalPolicy: r.approvalPolicy ?? undefined,
    status: r.status,
    payload: r.payload ?? undefined,
...
export function createSqliteRepositories(db: BetterSQLite3Database): SqliteRepositories {
  ...
  const projectRegistry: ProjectRegistryRepository = {
    ...
    resolveRef: (ref) =>
      run(() => {
        ...
        const rows = db.select().from(schema.projectRegistry).all();
    ...
    upsert: (entry) =>
      run(() => {
        db.insert(schema.projectRegistry)
          .values(entry)
          .onConflictDoUpdate({
  ...
  const parityReports: ParityReportRepository = {
    record: (report, recordedAt) =>
      run(() => {
        ...
        db.insert(schema.parityReports)
          .values({
  ...
  const writeReceipts: WriteReceiptRepository = {
    ...
    recordApplication: (row) =>
      run((): Result<void, DbError> => {
        db.insert(schema.writeApplications)
          .values({
            idempotencyKey: row.idempotencyKey,
            targetSystem: row.targetSystem,
            canonicalObjectKey: row.canonicalObjectKey,
