// Unit 2.4 — Postgres adapter (drizzle-orm/pglite | node-postgres) for every
// operational-store repository interface (§4 / REQ-D-002/003, §16 error convention).
//
// Deterministic + server-free: every case runs against `new PGlite()` (in-process
// REAL Postgres 16) with the schema materialized by the pg create-tables helper. The
// optional Docker-pg run (postgres:16 via node-postgres) is a separate, env-gated
// path (2.9) — not required here. The adapter NEVER throws across the boundary: every
// method returns a typed Result<T, DbError> (§16); failures are asserted as typed
// `err` values with an enumerable code, not caught exceptions.
//
// PARITY WITH 2.3: this suite mirrors `sqlite.test.ts` case-for-case — the Postgres
// adapter MUST be behaviorally identical to the SQLite adapter (the 2.9 contract suite
// runs the SAME tests against both). Coverage: ≥1 round-trip per repo plus the
// boundary behaviors the interfaces pin (append-only forward scan, exactly-once
// approval compare-and-set, outbox idempotency + due-query tombstoning, read-model
...
let repos: ReturnType<typeof createPostgresRepositories>;
...
// ── factory smoke ────────────────────────────────────────────────────────────
describe("createPostgresRepositories — factory surface (2.4)", () => {
...
describe("WorkspaceConfigRepository (postgres)", () => {
...
describe("EventLogRepository (postgres)", () => {
...
describe("WorkflowRunRefRepository (postgres)", () => {
...
describe("AuditRepository (postgres)", () => {
...
describe("ApprovalRepository (postgres)", () => {
...
describe("OutboxRepository (postgres)", () => {
...
describe("ConnectorCursorRepository (postgres)", () => {
...
describe("ProviderStateRepository (postgres)", () => {
...
describe("ReadModelRepository (postgres)", () => {
...
describe("GclProjectionRepository (postgres)", () => {
