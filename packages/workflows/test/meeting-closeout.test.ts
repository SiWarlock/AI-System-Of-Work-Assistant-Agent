...
describe("runMeetingCloseout — 13.8f-C: commit sibling entity-page plans after the meeting note's own commit", () => {
  ...
  it("not_armed_and_mint_failed_route_to_distinct_failureClasses — 13.8i-B: a precondition-HELD blocked mint reads differently from an ATTEMPT-errored one", async () => {
    ...
    const buildOutputsA = new FakeBuildOutputsPort({
      siblingPlans: [siblingPlan("sib-not-armed", true)],
    });
    await runMeetingCloseout(
      makeInput(),
      makeDeps({ health, buildOutputs: buildOutputsA, proposeKnowledgeApproval: notArmed } as never),
    );

    const mintFailed = new SpyProposeKnowledgePort({ kind: "err" });
    const health2 = new FakeMeetingHealthSink();
    const buildOutputsB = new FakeBuildOutputsPort({
      siblingPlans: [siblingPlan("sib-mint-failed", true)],
    });
    await runMeetingCloseout(
      makeInput(),
      makeDeps({ health: health2, buildOutputs: buildOutputsB, proposeKnowledgeApproval: mintFailed } as never),
    );
  ...
  it("zero_cards_rests_on_empty_plans_not_on_port_absence — the SAME bound port mints 0 when siblingPlans is empty and 1 when it is not", async () => {
    // ⭐ 13.8i-B: the default-boot zero-cards guarantee rests on meetingVault staying dormant (empty
    // siblingPlans), NOT on this port being absent — proven by holding the port instance CONSTANT and
    // varying only the plan set.
    const boundPort = new SpyProposeKnowledgePort({ kind: "ok" });

    const emptyOutputs = new FakeBuildOutputsPort({ siblingPlans: [] });
    await runMeetingCloseout(
      makeInput(),
      makeDeps({ buildOutputs: emptyOutputs, proposeKnowledgeApproval: boundPort } as never),
    );
    expect(boundPort.calls).toHaveLength(0);

    const nonEmptyOutputs = new FakeBuildOutputsPort({
      siblingPlans: [siblingPlan("sib-contrast", true)],
    });
    await runMeetingCloseout(
      makeInput(),
      makeDeps({ buildOutputs: nonEmptyOutputs, proposeKnowledgeApproval: boundPort } as never),
    );
