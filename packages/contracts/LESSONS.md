...
⭐⭐ **AMENDED 2026-08-24 (`### 24.97` leg (b) / `### 24.82` leg (b)) — the `Workspace` INSTANCE is now CLOSED BY CONSTRUCTION at 9.36; the CLASS survives elsewhere. Both re-derived at source, not inherited from this lesson's own prior text — per this lesson's own ⛔ two paragraphs up, a bare citation repair once manufactured a fresh false claim, so the cited mechanism was read directly before writing this.** `packages/db/src/adapters/workspace-read-gate.ts:75` runs `WorkspaceSchema.safeParse(row)` on every workspace read; both dialects route through it, and the sqlite call site cites this lesson by number in its own comment (`packages/db/src/adapters/sqlite/index.ts:363-365`: *"re-gate the stored row through the FULL WorkspaceSchema (never an unchecked cast) before it can re-cross into a write or a policy decision (contracts L76)"*). Closure here is **BY CONSTRUCTION, not by convention** (L103's family): `packages/db/src/schema/workspace-config.ts:26-45` types the nested `egressPolicy`/`providerMatrix` json columns `unknown`, NOT `Workspace["egressPolicy"]`/`["providerMatrix"]`, so a raw `$inferSelect` row no longer *structurally* satisfies `Workspace` for any consumer, present or future — that file's own comment: "a reader added tomorrow with no cast at all still cannot return the row as `Workspace`" (L70/L95 — pin the PROPERTY, not the call sites). Verified, not inferred: an `awk` census of `as Workspace` across `packages/db/src` returns **zero live casts** — its only two surviving hits are both *inside comments* narrating the pre-9.36 history (`workspace-read-gate.ts:30`, `workspace-config.ts:39`), not code.
⛔ **THE CLASS IS NOT CLOSED — six OTHER row types cross the identical boundary unparsed, measured at the same commit (`16169caf0a0e4575caa7b8faae2e6bbd8ddd2911`) by TWO INDEPENDENT INSTRUMENTS (`grep -rnE` and a per-file `awk`, both agreeing): `ConnectorInstanceRow` (×8: sqlite:553,562,593,606 · postgres:574,582,614,628), `CrossWorkspaceLinkRow` (×6: sqlite:626,640,657 · postgres:649,662,680), `SourceDispositionRow` (×6: sqlite:705,714,730 · postgres:725,734,743), `WorkflowRunRef` (×8: sqlite:802,811,822,836 · postgres:829,839,849,867), `ProviderProfile` (×6: sqlite:1161,1163,1195 · postgres:1203,1205,1236), `GclProjection` (×6: sqlite:1254,1277,1286 · postgres:1299,1325,1333) — **40 unparsed read-path casts total (20 sqlite + 20 postgres)**, every one a `row ? ok(row as <Type>) : err(...)`, `.all()/.select() as <Type>[]`, or `rows[0] as <Type>` shape the pre-9.36 `row as Workspace` also had.**
⚠ **The first pass at this count was WRONG and is corrected here rather than silently replaced (this lesson's own [L71](#71)/[L93](#93) discipline): a narrower `"row as "`-prefixed search returned 27 and a false "`SourceDispositionRow` has no postgres equivalent" — both defects had the SAME cause, an undercounting search pattern that misses the `.all()`-chained and `rows[0]`/array-cast forms (`.all() as ConnectorInstanceRow[]`, `rows as GclProjection[]`, `rows[0] as SourceDispositionRow | undefined`, …), which do not start with the literal substring `"row as "`. Re-run with the broader `as <TypeName>` pattern below and cross-checked against an independent `awk` pass before this number was written.**
`GclProjection` is the sharpest of the six: `packages/policy/src/visibility.ts`'s own `### 24.81` note (same round) traces a `GclProjection` returned from `admitProjection`'s ALLOW branch straight into `repo.upsert` on the WRITE side of the identical repository whose `get`/`listByWorkspace`/`listByVisibility` cast the row back out unparsed on the READ side — this lesson's read-path dual, recurring one type over.
`pattern: grep -rn "as Workspace\|row as " packages/db/src` — SUPERSEDED for the `Workspace` row specifically (0 live hits as of the 2026-08-24 amendment; do not re-open on this pattern alone) — and this narrower pattern is also the one that undercounted the class below, so do not reuse its shape for a fresh census either. **Re-pointed at the row types still cast-typed:** `grep -rnE "as (ConnectorInstanceRow|CrossWorkspaceLinkRow|SourceDispositionRow|WorkflowRunRef|ProviderProfile|GclProjection)" packages/db/src` — each hit is a read whose consumers must parse before writing (or, for `GclProjection`, before re-entering the visibility gate — see `### 24.81`).
...
⭐⭐ **AMENDMENT 2026-08-12 — THE MISSING OPERATIONAL HALF: HOW A RECIPIENT DISCRIMINATES. Banked at the lead's instruction, who named the gap precisely: *"L116 says authority attaches to the speaker, not the claim — but it never said how a RECIPIENT can discriminate."*** Everything above is **diagnostic** — it tells the relayer to mark a relay and tells the recipient to verify a premise, but a recipient staring at one message has had **no test** for telling a genuine ruling from plan prose wearing lead authority. The tell offered above (*"a real ruling carries a REASON"*) **fails exactly when the prose already contains the reason** — which is the common case here, because rulings are routinely made *about* tracked work whose tracker entry already states its rationale.

**The instance:** the orchestrator dispatched `24.44` attributing its no-red-window Done-when to *"the lead."* ⛔ **That exact wording already existed verbatim in `### 24.44`'s own `Done-when` line in `IMPLEMENTATION_PLAN.md`.** The implementer — correctly — **refused to start**, because from where it sat, *a fresh lead ruling sequencing the pair* and *the plan's existing Done-when relayed with lead authority attached* were **indistinguishable, and they are different facts**: the first re-assigns the implementer and supersedes three prior standing instructions; the second means **nobody had actually moved it off its previous slice**, and it would have been acting on an authority that was never exercised.

⭐ **THE DISCRIMINATOR — CITE THE PART OF THE RULING THE PROSE CANNOT CONTAIN.** Here that was the **ASSIGNMENT**: the lead named *the pair, the file, and the line* — `apps/worker/src/composition/crossWorkspaceRead.ts:139`, one half to knowledge, one to worker. ⭐ **A document can state a requirement, a rationale, a Done-when, even a sequencing constraint — it cannot assign a specific teammate to a specific half at a specific moment.** ⇒ **prose can be quoted; a dispatch cannot be forged from it.**

⇒ **RELAYER'S OBLIGATION (concrete, replaces "mark a relay as a relay" when the two overlap): when a ruling's content overlaps text that already exists in a durable doc, do NOT quote the overlapping part as your evidence — cite the non-overlapping part.** Naming the requirement proves nothing; naming *who was assigned what, when, by whom* proves the ruling happened. ⚠ **If you find that every part of the "ruling" also exists in the doc, that is the finding: you are relaying, and you should say so.**

⇒ **RECIPIENT'S TEST:** ask *"is there anything in this instruction that the tracker could not have said on its own?"* — no ⇒ treat it as a pointer and verify before acting; yes ⇒ that part is the ruling, and the rest rides on it.

⭐ **Why this belongs on L116 rather than standing alone: it converts a lesson that could only be applied in retrospect into one a recipient can apply at the moment of decision** — and the applying party here was an **implementer**, the furthest spoke, which is where the diagnostic form was least usable.
...
⛔⛔ **AMENDED AND CORRECTED 2026-08-18 (LEAD RULING) — MY FIRST AMENDMENT TO THIS LESSON PRESCRIBED THE WRONG REMEDY, AND THE CORRECTION IS THE INTERESTING PART.**

**I amended this lesson hours earlier with:** ~~*"the compile fix must land AFTER the change that requires it"*~~ — after authorizing a crossing that landed **first** and left `packages/workflows` red at HEAD. ⛔ **That amendment treats an ORDERING BUG as an ordering PROBLEM. The lead's ruling goes one step back: there was no ordering to get wrong until the two commits were SPLIT.**

⇒ ⭐⭐ **THE RULE, in the lead's form:** ***A FORCED CONSEQUENCE RIDES WITH ITS CAUSE. Where the consequence is a COMPILE dependency, SAME-COMMIT is mandatory — separation is an ordering bug waiting to happen. Attribution is a MESSAGE problem, and it is always solvable in the message.***

⛔ **My mechanics — *"own commit, pathspec-limited, message naming the upstream slice"* — bought ATTRIBUTION and sold ATOMICITY**, and only the first of those was ever at risk. **A foreign hunk riding INSIDE the slice commit, with the message naming the file and stating *"mechanical, forced by this change, no logic touched,"* is just as attributable and CANNOT INVERT.**

⭐⭐ **THE EVIDENCE THAT THE RULE IS GENERAL, AND IT IS WHY THIS IS BANKED RATHER THAN JUST FIXED: the same lead had ruled TWO HOURS EARLIER, on `### 24.93`, that a tracker edit MUST land in the SAME COMMIT as the code that expires it — for exactly this reason.** ⇒ ***the opposite mechanics, applied to a compile dependency, produced precisely the failure the first ruling was written to prevent.*** ⚠ **Two people reasoned carefully and reached opposite mechanics within two hours — that is a rule UNDER-SPECIFIED, not a person being careless** (the `L141`-amendment shape, applied to commit mechanics).

⚠ **Scale, stated because the milder description understates it: the red was at HEAD across THREE commits, not for a moment.** *"I left it red and fixed it next"* is the description-naming-only-the-milder-half shape this round has been strict about.
...
<a id="138"></a>
## 138. A PROPAGATION GAP EXPRESSED AS DUPLICATED CODE IS WORSE THAN ONE EXPRESSED AS DUPLICATED PROSE — nothing reads a code fork until it's armed

**Date:** 2026-08-11. **Origin:** `24.32` — `proposeWindows.ts`'s unfixed fork of `24.19`'s hardened raw-content-shape check, established dormant by the lead the same day.

**The instance.** `24.19` hardened `isRawContentShaped`/`carriesRawContent` in `packages/contracts/src/models/gcl-projection.ts` (Map/Set, non-enumerable, and Symbol-keyed bypasses all closed, same day). `packages/workflows/src/activities/proposeWindows.ts` maintains an **independent, unfixed implementation of the identical check** — same `Object.entries`-based traversal, same three bypasses, none fixed. Its own doc comment calls it *"the LOAD-BEARING Flow-3 leakage check"* gating the Tool Gateway. Reachability established (grep-and-classify, every hit read): **zero production callers** — dormant, real only once Phase 25's cross-calendar activity is wired.

⭐ **THE MECHANISM, and why it's a sharper instance of this round's own headline than any prior one:** every earlier propagation-gap instance this round (`LES-1`'s `L101` correction, `24.30`'s reclassification comment, the lessons-ledger-vs-tracker gaps) was a correction failing to reach **prose** — a doc, a ledger, a comment. **This one is a correction failing to reach a SECOND IMPLEMENTATION of the same safety predicate.** ⇒ **A propagation gap in code is structurally worse than one in prose: nobody reads a dormant code fork until the moment it's wired live, by which time the "LOAD-BEARING" comment has already told a code-only reader the path is covered** — the false-liveness pattern from the GCL work, recurring one module and one week later, in a fork nobody knew to check because nothing pointed at it.

⚠ **The fix is NOT to re-harden the duplicate** — that repeats the propagation failure at one remove (two hardened copies now drift independently the next time either changes). **The fix is to delete the fork and import the shared predicate** — the same move (L39: reuse over re-derive) already applied twice this round on different predicates. A safety-critical traversal function should have exactly one implementation, exported, reused — never re-derived per call site.

⇒ **Do:** when a security-relevant check is found duplicated across packages, the remedy is deletion-and-reuse of the hardened original, not independent hardening of the copy. And: **whenever a hardening fix lands on a named predicate, grep the codebase for OTHER implementations of the same concept** (not just other CALLERS of the fixed one) before considering the fix complete — a hardened original with an unfixed twin is not a closed finding, it's a finding that hasn't finished being found.

`pin: none — the trigger is a duplicated SAFETY predicate, not a source shape a mechanical check can classify (structural similarity ≠ semantic identity).` `pattern: grep -rn "isRawContentShaped\|carriesRawContent\|payloadCarriesRawContent" packages apps — enumerate every implementation of this concept, not just this fork's.` `accepted: partially enforceable` — the grep finds literal name reuse; a differently-named third fork would not surface without a concept-level review.

---

<a id="139"></a>
## 139. A REVIEWER'S EMPIRICAL MUTATION TEST (`git checkout --` to break-then-restore) IS UNSAFE IN A SHARED NON-WORKTREE CHECKOUT

**Date:** 2026-08-11. **Origin:** `24.30`'s Step-9 review — both reviewers independently ran the project's own recommended empirical-verification methodology (mutate the fix, confirm it breaks, restore) against the live file in the team's single shared checkout (no per-implementer worktree this round).

**The instance.** One reviewer's `git checkout --` transiently reverted the file under test to its pre-fix state — correct methodology, applied to a file that isn't isolated. The revert was confirmed fully restored afterward (diff stat, typecheck, tests), and the implementer independently re-verified before reporting. **No harm resulted this time.** But for the window between the revert and the restore, any OTHER teammate's concurrent read, test run, or commit in this same tree would have observed a file silently missing its just-implemented fix — with no signal that it was mid-mutation-test, not actually broken.

⭐ **THE MECHANISM: this project explicitly recommends empirical mutation-testing (mutate → confirm RED → restore) as the standard for proving a guard actually works — it is correct practice, cited approvingly throughout this round's own lessons (L100's fourth mechanism, `24.19`'s adversarial pins, `24.29`'s own mutation-verified fix).** The methodology assumes the file under test is effectively private for the duration of the mutation. **In a single shared checkout with multiple concurrent implementers (this round's actual topology), that assumption is false** — the same shared-tree hazard this project has banked repeatedly for COMMITS (three-step commit discipline, pathspec-limited commits, "commit promptly so the window is seconds") applies equally to a reviewer's *local, uncommitted* mutation test, which was not previously named as needing the same discipline.

⭐⭐ **AMENDED 2026-08-13 — THE INVERSE DIRECTION, AND IT IS THE ONE A THIRD PARTY CAUSES.** `L139` as written covers a reviewer **mutating** shared files. The mirror image is **someone else TIDYING files that belong to a running agent's working set.**

**The instance:** the orchestrator flagged an untracked `packages/policy/test/tmp-sec/poc.test.ts` to the implementer as probable throwaway scratch to dispose of before Step 10. ⛔ **It was not the implementer's. It was their CURRENTLY-RUNNING `security-reviewer`'s attack harness — two files, written minutes earlier, with the agent still working.** **The implementer refused to delete them mid-run** — *"that would destroy a running reviewer's working set mid-measurement"* — and was right.

⛔ **THE MECHANISM: NOTHING IN A WORKING TREE DISTINGUISHES *"abandoned scratch"* FROM *"a live harness belonging to a process still running."*** Both are untracked, both look temporary, both are named like throwaways — **and the second is destroyed by exactly the hygiene the first invites** (`L117`'s *"an untracked file is the only real loss vector"* pointing the wrong way: **true, and it makes a well-meant cleanup a data-loss event**).

⚠ **The orchestrator's compounding error was ATTRIBUTION BY TERRITORY: an untracked test file in providers' tree during providers' slice was assumed to be providers' authorship.** ⭐ **The mtimes were checkable and `stat` would have settled it in one command** — `L159` with the orchestrator as subject: **a cause proposed without stating how it could be ruled out.**

⇒ **DO. Before flagging or disposing of any untracked file you did not create: check whether a subagent is IN FLIGHT in that area, and address the disposition to the OWNER rather than proposing the action.** ⭐ **"Decide its fate" is safe; "this looks like scratch, clear it" is not** — and in a team that dispatches reviewer subagents, the window in which a harness is live is exactly the window in which it looks most disposable.

`accepted: not mechanically enforceable` — **enforcement point (both directions): any mutation test, and any cleanup or flag concerning untracked files in a shared checkout while other agents or subagents are running.**

⇒ **Do:** a reviewer running an empirical mutation test in a shared non-worktree checkout should (1) keep the mutate→confirm→restore window as short as possible — do it as the last verification step, not an early one, so nothing else queues behind it; (2) confirm restoration immediately and explicitly (diff stat + a targeted re-run), not just trust `git checkout --`'s own exit code; (3) if concurrent implementers are active in the same tree (the common case this round), consider stating in the review "mutation-testing now, file X briefly reverted" so a teammate mid-test on that exact file knows to hold. **Structural fix, if this recurs:** per-implementer worktrees remove the hazard entirely (already recorded elsewhere in this project as "recorded-not-done") — this lesson is the concrete cost of not having them, not a reason to abandon mutation-testing.

`pin: none — the trigger is a review-time git operation in a shared tree, not a source pattern.` `pattern: none — process convention.` `accepted: not mechanically enforceable` — enforcement point: any reviewer about to run `git checkout --`/`git stash`/similar on a file inside a shared, non-worktree, multi-implementer checkout.

---

<a id="140"></a>
## 140. KNOWING THE DEFECT CLASS, AND HAVING JUST FILED THE TASK TO FIX IT, DOES NOT PREVENT COMMITTING IT AGAIN AN HOUR LATER

**Date:** 2026-08-12. **Origin:** `24.33`, main-orchestrator — self-committed, owner-caught, the sharpest instance of `L131` this round produced.

**The instance.** The orchestrator filed `24.29` — the task fixing `24.13`'s reachability census being scoped to `apps/` only, when the safety claim it backs is repo-wide. **About one hour later, on a different task (`24.33`), the same orchestrator filed a reachability finding using the identical `apps/`-only grep** — and, on that evidence alone, called the finding "CONFIRMED UNREACHABLE." It was not: the real caller (`guardCrossWorkspaceRawRead` ← `authorizeCrossWorkspaceRawRead` ← `cross-workspace-links.ts`) lived in `packages/`, exactly the directory `24.29` exists to add to the scan. The owner caught it on independent re-read; the orchestrator's own re-verification (prompted by the catch, not self-initiated) then found a SECOND error in the same filing — a cited function name (`reconcileProjection`) that does not exist in the file at all.

⭐ **THE MECHANISM, and it is L131's claim proven a second time with a sharper instance: recording a defect does not stop its false signal being believed, EVEN BY THE PERSON WHO RECORDED IT, EVEN WITHIN THE HOUR, EVEN ON A TASK ABOUT THE SAME SUBSYSTEM.** Writing `24.29`'s task body did not install a durable habit — the pattern that produced the original mistake (`grep ... apps/` and trust the result) was still the orchestrator's default reflex the next time reachability needed checking, because nothing about filing the task changed the REFLEX, only the RECORD.

⚠ **Why this is stronger evidence than the lead's own parallel instance (an accusation later withdrawn) or `L131`'s original framing (outcome-phrasing, not verification habit): here the SAME PERSON who diagnosed the class, in writing, an hour earlier, reproduced it — with full knowledge of the fix on the very same board.** A lesson banked does not immunize its author; it only gives everyone else a name for what already happened to them.
...
<a id="146"></a>
## 146. When you correct a wrong value, ask what its wrongness was incidentally PREVENTING — a defect can be load-bearing without anyone knowing it

**Date:** 2026-08-12. **Source:** `24.15`'s Step 9, found INDEPENDENTLY by both reviewers; `security-reviewer`'s framing, banked at the lead's instruction as its own lesson rather than a line in the resulting task.

**The instance.** `rebuildAction` reconstructed a held outbox action with a hardcoded `REDRIVE_APPROVAL_POLICY = "queued"` literal, because `OutboxEntry` never persisted the original token. That is a real defect — an action that was auto-eligible when proposed came back needing approval — and `24.15` correctly fixed it by persisting and reading back the true token.

⛔ **What nobody had noticed: the wrong literal was ALSO, incidentally, the reason `requiresApproval`'s `autoAllowed` branch was STRUCTURALLY UNREACHABLE on every redrive.** Since `"queued" !== "auto_private"`, the auto-eligible path could not be entered, so the conjuncts behind it were never evaluated — **including a `ResolvedWorkspacePolicy` that is bound ONCE per composition rather than re-resolved per entry.** ⇒ fixing the literal **un-masked** a latent posture-binding gap in which a drain processing entries across workspaces could evaluate an **employer-work** entry's auto-eligibility under a **personal** workspace's posture (safety rule 4). The reviewer's sentence, verbatim: ***"the hardcoded literal was also, incidentally, the reason a latent posture-binding defect one layer out was unreachable — removing it un-masks that. That is the risk-direction flip."***

⭐ **THE MECHANISM: a defect can be doing safety work that nothing records, because nobody knew the guarantee existed to write it down.** The `autoAllowed` branch was unreachable **by accident**, so no test pinned it, no comment claimed it, and no ledger listed it — it was a **free, untested, but REAL guarantee**. Removing the defect removes the guarantee, and **nothing anywhere reports the loss**: no test reds (the branch had none), no type changes, and the fix's own direction is fail-*safe*, so every signal says the change was an improvement.

⛔ **This is the INVERSE of the round's dominant shape and it is rarer and harder to see.** That round produced many remedies that rebuilt their parent defect (a fix re-importing `L134`'s shape; a narrowing that retired its own fixtures' discrimination — `L79`). **This is the opposite: a correct remedy that DELETES an unrecorded protection.** A remedy re-creating a known defect is at least a known shape someone can look for; **an unrecorded protection has no name to search for.**

⇒ **DO, and it is one question at Step 2.5 of any value-correcting slice: *what was the wrong value incidentally preventing?*** Concretely — **if the wrong value made a branch, a state, or a code path unreachable, name that path and ask what guards it once the value is right.** ⚠ **The tell is a fix whose direction is fail-safe and whose blast radius is "one literal"**: that combination suppresses scrutiny exactly when a reachability change is hiding underneath it.

⭐ **DISPOSITION MATTERS AS MUCH AS DETECTION — file the un-masked gap as a WIRING PRECONDITION, not a follow-up** (this became `### 24.50`, class of `24.9`/`24.33`). **As an ordinary task the exposure is guarded only by nobody having wired the consumer yet — safe by dormancy, which is not a guarantee (`L106`). As a precondition it is safe BY CONSTRUCTION: the consumer cannot be wired without closing it.** ⇒ that is what lets the correct fix ship immediately instead of being held.
...
⭐⭐ **AMENDED 2026-08-14 (`### 24.85` Step 9, worker-implementer, self-reported) — TWO SHAPES FROM ONE SLICE; the lead asked for both.**

**(a) DISCARDED EVIDENCE, NOT MISSING EVIDENCE — SECOND INSTANCE, the companion clause above with a new subject.** Worker's note claimed *"every fake `ApplyPlanFn` declares exactly one parameter."* **Measured: 5 fakes — THREE declare ZERO, two declare `(command)`.** ⛔ ***They had the disconfirming counts in their OWN earlier grep output and wrote a uniform claim over them.*** ⭐ **The correction is STRONGER than the original: *"no fake declares a SECOND parameter"* is both TRUE and the property that actually matters** — the uniform claim was neither. ⇒ **when a sentence generalises over a set you have already enumerated, re-read your own output before writing the generalisation. The tidy sentence is what overwrites the ragged data**, and it does so silently because a generalisation *feels* like a summary of what you just saw.

**(b) A CORRECT CONCLUSION CAN SHIP WITH AN OVER-BROAD JUSTIFICATION — AND THE JUSTIFICATION IS THE REUSABLE PART.** The orchestrator approved the reading *"green means NOT READ, never did-not-run."* ⛔ **Over-general at that seam: `createCommitActivity` wraps `applyPlan` in a §16 catch folding throws to `commit_failed`, so the `commit_failed`-asserting tests stay green EVEN IF a throwing check WERE read.** ⭐ **The probe still discriminated IN AGGREGATE — the ok-asserting tests carry it — so the CONCLUSION held and only the stated REASON was too wide.** ⇒ ⭐⭐ ***which is precisely why it matters: nobody re-derives a conclusion, but everybody REUSES a justification.*** **A blanket sentence is the thing a future reader quotes to skip re-measuring** (`L145`'s decay entering through the rationale rather than the claim). ⚠ **Review naturally asks whether the ANSWER is right and almost never whether the REASON is narrower than stated** — the answer is checkable in one command; the reason's scope is not. ⇒ **state the seam a mutation result generalises over, and say where it stops.** ⚠ **Found by the implementer against reasoning the orchestrator had already APPROVED, which is the only direction this one travels.**
...
⭐⭐ **AMENDMENT 2026-08-17 — ONE MORE HOP, AND IT IS THE WORST DIRECTION: A CORRECTION CAN INTRODUCE A NEW DEFECT OF THE SAME FAMILY.** (contract-implementer, `### 24.84`, self-reported.) A security reviewer flagged an **`UNREPRESENTABLE` over-claim** (`L161`). **The fix for it added a docblock prescribing a bypass census — `grep -rnE 'as (unknown as )?WorkspaceId\b'`.** ⛔ **That command returns ZERO on the one file in the repo carrying a deliberate named bypass**, because every cast there is spelled through **indexed access** (`GclProjection["workspaceId"]`, `Workspace["id"]`). ⇒ ***the remedy for an over-claim about a population was itself a claim about a population, asserted more confidently than its instrument supported.***
⇒ ⛔ **THE RULE: A CORRECTION IS NEW CONTENT AND CARRIES THE FULL EVIDENTIARY BAR — it does not inherit credibility from the defect it repairs.** ⚠ **And it is the hardest case to catch, because the correction arrives with a REVIEWER'S ENDORSEMENT ATTACHED and reads as already-vetted.** The reviewer approved *retiring the over-claim*, not *the census that replaced it*.
⭐ **WHAT MAKES THE REPAIR EXEMPLARY, and it is the reusable half: they WITHDREW the command as a census rather than patching it to catch one more spelling.** **Patching would have been `L82`'s forbidden middle — the miss-set would still have been unknown.** The docblock now states the PROPERTY rather than a pattern, records that an earlier draft prescribed a spelling census **and what it measured wrong**, names indexed access as the family missed, marks both greps **STARTING POINTS with an unknown miss-set**, and requires the unit with any reported number. ⇒ ***the docblock carries the FAILURE, not just the corrected answer — a reader who trusts it needs to know it was wrong once and in WHICH DIRECTION, or they will trust the next command in it exactly as hard.***
⭐⭐ **SECOND INSTANCE OF THIS HOP THE SAME DAY, DIFFERENT AGENT, AND IT IS TIGHTER: knowledge-implementer's fix for a comment-versus-reality defect RENAMED a test — and left the helper docblock at `:258` citing the OLD name.** ⇒ ***a comment pointing at a test that no longer existed, created by the repair for a comment-versus-reality defect.*** ⛔ **Found by their own post-edit verification, not by review** — they checked the fixes took rather than assuming, and reported it as *"the first prose defect in this slice introduced by a correction rather than surviving one."*
⇒ ⭐ **ACTIONABLE COROLLARY, and it is mechanically checkable unlike most of this entry: A RENAME OWES A CITATION SWEEP.** **Renaming a test, symbol or file invalidates every prose reference to the old name, and prose references are exactly what no compiler and no suite will catch.** ⚠ **Verify by count in both directions — `0` occurrences of the old name AND a non-zero count of the new one** — a single-direction check passes on a rename that silently landed nowhere.
⚠ **AND THE SCOPE CALL WAS THEIRS AGAINST THE ORCHESTRATOR'S, CORRECTLY: the orchestrator said "not urgent, not this slice."** ⛔ **Wrong — the line was introduced BY THAT SLICE, an hour earlier, in a file the slice already modifies.** ⇒ **deferring would have landed a known-false-assurance instrument as authoritative in the artifact a reader meets first.** ***`L82` offers make-it-true or withdraw-it, and there is no third option that survives a landing.*** **Cleaning up a mess you made this slice is not scope expansion.**
...
⚠ **DISTINCT FROM `L51`, AND THE REFINEMENT MATTERS.** `L51` says close-out debt belongs in a **file**, never only in a head or a message — that is about **DURABILITY**. This is about **LOCATION**: *which* file, and *where in it*. ⛔ **A perfectly durable ruling filed in the tracker still fails if the tempting edit happens in `ARCHITECTURE.md` and the editor never opens the tracker.** ⛔⛔ **NARROWED 2026-08-17, AND TODAY SUPPLIES THE COUNTER-EXAMPLE THAT FORCED IT.** This entry first read *"durable and un-encountered is indistinguishable from absent."* **That is TOO STRONG, and `### 24.84` refutes it hours after it was written:** that reason was durable and un-encountered by **two** people who searched for it — **and it was NOT absent. contract-implementer recovered it by CONTENT search.** ⇒ ***had it truly been absent, that search returns nothing and the owner's authorization stays hollow*** — which is precisely what the lead told the team for an hour before the recovery. **The distance between durable-un-encountered and absent is the distance between a recoverable error and an unrecoverable one, and this team traversed it in one session.**
⭐ **CORRECTED FORM: durable-but-un-encountered is indistinguishable from absent AT THE MOMENT OF THE WRONG ACTION — and not otherwise.** It still supports later audit, reconstruction, and third-party recovery, which is not nothing; it is the difference between the two outcomes above.
⇒ ⛔ **`L51` AND `L187` ARE COMPLEMENTS AND NEITHER SUBSTITUTES FOR THE OTHER: the in-file guard buys you THE DECISION MOMENT; the durable home buys you EVERYTHING AFTER.** ⚠ **The tempting misreading of this lesson is *"so put it where it is read, not where it is filed."* BOTH. ALWAYS BOTH.**
...
⛔⛔ **BOUND — DO NOT LET A THIRD INSTANCE PROMOTE THIS TO A VALIDATED PRACTICE. A COUNT IS NOT A VALIDATION.** Both recorded successes were prose defences against a **specific, foreseeable** edit. ⚠ ***There is NO instance of this working where the wrong action was not predictable in advance — and that is the case where it would matter most.*** **Recorded as two instances, not as a practice**, and a future editor adding a third should add it as a third instance rather than as evidence the pattern generalises.
...
⛔⛔⛔ **AND THE DECISIVE AMENDMENT, FOUND BY providers-integrations AFTER THE FACT — THE COLLISION HAD ALREADY HAPPENED, SILENTLY, AND SURVIVED THREE WEEKS.** **A duplicate-check across ALL committed session docs (not just this round) found `docs/sessions/114-…` EXISTS TWICE** — `114-…-arc4-living-vault-keystone-13.8bcd-knowledge.md` and `114-…-desktop-arc1-phase9-calendar-obsidian-priorities.md`, **committed by two different areas and still duplicated at HEAD ~3 weeks later. Verified independently at HEAD.**
⇒ ⭐⭐ ***DETECTION SCALES WITH COLLISION MULTIPLICITY, NOT WITH ANY CONTROL. Today's THREE-way collision was caught because the noise was unmissable. `114` was a TWO-way collision, it was silent, and nobody noticed it for three weeks.*** ⛔ **The convention has NO DETECTOR — we have been relying on collisions being loud enough to trip over.**
⚠ **AND THE RECORD ITSELF MISLEADS: session `174`'s renumber banner presents the `173` collision as the FIRST occurrence. It was not — `114` predates it by three weeks.** ⇒ ⭐ **an *earliest-known* written as an *origin*, which is `L190`'s family: the claim is true of what the author had seen and false of the population.**
✅ **GUARD INSTALLED 2026-08-18 (providers-integrations' proposal, adopted): `scripts/plan-lint.sh` now reports duplicate `NNN` prefixes in `docs/sessions/`.** ⭐ **One `sort | uniq -d`, and it needs NO CONCURRENCY ASSUMPTION AT ALL — precisely the property the `max+1` convention lacks.** **Non-vacuity control run: it fires on the known `114` instance.** ⚠ **WARN not FAIL, deliberately — `114` is a live pre-existing duplicate whose rename would break inbound links, so failing would block every tracker commit. Promote to a violation once `114` is dispositioned (`### 24.107`).**
