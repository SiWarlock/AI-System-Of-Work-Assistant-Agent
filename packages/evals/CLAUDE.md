...
# System of Work Assistant `packages/evals/` — Build Guide

> **You're in `packages/evals/`.** This file plus root `CLAUDE.md` both load. The root file covers global project conventions + shared comm rules (track-prefix, escalation taxonomy, messaging budget); this file owns code-area conventions for eval & test harness.
...
| eval & test harness code | `packages/evals/` | this `CLAUDE.md` + root |
...
1. **Implementer runs `/session-end`.** Implementer writes ONLY:
   - `packages/evals/` code files (the slice's implementation)
   ...
   **Implementer must NOT touch (all orchestrator territory).** *This list is the canonical statement
   ...
   - `packages/evals/LESSONS.md`
   - `packages/evals/CLAUDE.md` (entire file — both the Cross-doc invariants table AND the Lessons logged index)
   - `ARCHITECTURE.md`
...
| Lessons logged (full prose) | `packages/evals/LESSONS.md` | by lesson # |
...
- **Stack:** Vitest · EVAL-1 harness · conformance + leakage/injection suites · perf benchmark
- **Validation:** Zod + JSON Schema (ajv)
- **Lint / types / tests:** ESLint / tsc --noEmit / Vitest
- **Territory (this track owns):** `packages/evals/`
...
1. **Write code without a failing test first** (the harness itself is deterministic).
2. **Put timing/latency assertions in the per-slice RED/GREEN loop** — they're flaky; the perf benchmark runs as its own task at its own cadence vs the ARCHITECTURE budgets.
3. **Mock a load-bearing path that the DoD certifies** — final DoD requires the real integrations (no permanent stubs; Postgres passes the same repository contract suite as SQLite).
4. **Let an acceptance suite drift from its PRD §20.1 test** — every §20.1 test maps 1:1 to a named suite/fixture.
5. **Under-size the EVAL-1 / leakage corpora below the floors** — meeting-closeout ≥20 labeled transcripts, retrieval ≥30 queries, leakage ≥15 cases.
...
```
packages/evals/   EVAL-1 corpora + harness · provider/runtime/adapter conformance · prompt-injection red-team · WS-7 leakage suite · perf & latency benchmark
```

Layer dependency direction: `packages/evals → (all subsystem outputs) → {domain,contracts}`. Consumes contracts + every track's output; defines no new contract surface.
...
The full prose for each lesson lives in `packages/evals/LESSONS.md`. This index is the compact orientation surface.
