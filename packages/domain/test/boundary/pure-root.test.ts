...
// ── 13.22 — the two pure-root-scan.ts helper copies are pinned equivalent ───
// packages/contracts/test/_helpers/pure-root-scan.ts and this package's own
// packages/domain/test/_helpers/pure-root-scan.ts are INTENTIONALLY duplicated
// verbatim (13.20 — see that file's own header: neither package's test suite
// reaches across the seam to police the other). All package-specific config
// (pkgSrcRoot, allowedInternalPackages) is passed in as ARGUMENTS by each
// package's own pure-root.test.ts, so there is no legitimate reason for the
// two copies to differ at all. A silent divergence is the failure this guards:
// someone fixes a scanner bug in one copy and not the other, and the OTHER
// package's boundary test keeps passing against a stale, weaker scanner,
// never going red — the guarantee gets quietly smaller without ever failing.
//
// Hosted HERE (domain's suite), not contracts', because domain already
// depends on contracts (13.20's own ALLOWED_INTERNAL above) — reading a file
// from the direction domain -> contracts adds no new dependency edge; the
// reverse would. This assertion polices the test HELPERS' equivalence, not
// the other package's purity: reading a file's TEXT via node:fs is not an
// import, so it creates no dependency edge and cannot trip either copy's own
// §2.5 scanner (whose PKG_SRC_ROOT covers only src/, never test/) — 13.20's
// "neither suite reaches across the seam" principle is about imports coupling
// behavior, and this does neither.
//
// Non-vacuity: the risk here isn't "the comparison always passes" (there is no
// complex control flow to trivially satisfy) — it's "the comparison might be
// comparing one file to itself" (e.g. a copy-pasted path). The path-distinctness
// + both-files-exist assertions below close exactly that mode BY CONSTRUCTION,
// so the content comparison fails if and only if the contents actually differ.
// That structural closure is what makes an on-disk mutate/revert unnecessary —
// unlike 13.20's own discovery/detection proof, there's no safe in-memory
// substitute for a real mutation here, so the better fix is removing the need
// for one rather than performing it against a shared, concurrently-worked tree.
// ⚠ The bound on that claim: the distinctness check is LEXICAL (string
// comparison of two path literals), not filesystem-identity — a symlink
// aliasing the two paths would defeat it. A `realpath`/inode comparison would
// close that too; not done here, because no such symlink exists today and
// "structurally closed" should not overclaim past what the check actually does.
//
// ⛔ IF A FUTURE SLICE LEGITIMATELY DEDUPLICATES THE TWO COPIES (Q3's rejected
// option, reconsidered later): DELETE THIS TEST ALONG WITH THEM. Do NOT
// "fix" a resulting red by retargeting both readFileSync calls at the same
// path — that satisfies path-distinctness's opposite (making both paths equal
// defeats the assertion meant to catch exactly this) and restores a green
// check over an absent guarantee: the same defect class this test exists to
// close, regenerated by its own remedy.
describe("the two pure-root-scan.ts copies do not diverge (13.22)", () => {
  it("the_two_pure_root_scanners_do_not_diverge — contracts' and domain's copies are byte-identical", () => {
    const contractsPath = join(REPO_ROOT, "packages/contracts/test/_helpers/pure-root-scan.ts");
    const domainPath = join(REPO_ROOT, "packages/domain/test/_helpers/pure-root-scan.ts");

    // Non-vacuity: prove the two reads target two DIFFERENT real files before
    // trusting their content to agree — a comparison of a file with itself
    // would pass trivially regardless of what either scanner actually contains.
    expect(contractsPath).not.toBe(domainPath);
    expect(existsSync(contractsPath), `missing: ${contractsPath}`).toBe(true);
    expect(existsSync(domainPath), `missing: ${domainPath}`).toBe(true);

    const contractsCopy = readFileSync(contractsPath, "utf8");
    const domainCopy = readFileSync(domainPath, "utf8");
    expect(domainCopy).toBe(contractsCopy);
  });
});
