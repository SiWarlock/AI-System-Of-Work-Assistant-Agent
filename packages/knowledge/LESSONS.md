...
## <a id="2"></a>2. Strict `=== true` on an external-client-returned boolean at a trust-gate floor — a truthy check at an I/O boundary is a false-green vector (worker Lesson 28's arming-guard pattern, applied in the knowledge layer)

**Date:** 2026-07-15.
**Source slice:** 13.10 knowledge false-green hardening `e808a43` (brief 073) — the `rebuildIndexFromMarkdown` wholesale-replace gate (`rebuild.ts:160`).
